/**
 * E2E tests for React 19 integration — Directives.
...
test.describe('React + WCC Directives', () => {
  ...
  test.describe('Test 12: Conditional (if/else-if/else)', () => {
    test('shows "Visible" when visible is true', async ({ page }) => {
      await page.goto(BASE + '/#/directives');
  ...
  test.describe('Test 14: Input (prop + event)', () => {
    test('typing in input updates display and React state', async ({ page }) => {
      await page.goto(BASE + '/#/directives');
      const input = page.locator('#test14 input');
      await input.fill('hello react');
      await expect(page.locator('#test14 span')).toContainText('hello react');
      await expect(page.locator('text=React value:').first()).toContainText('hello react');
    });
