    ...
    fc.assert(
      fc.property(
        ...
        (itemVar, source, signalName) => {
          ...
          expect(output).toContain("this.__for0_anchor = findAnchor(__root, 'each', 0)");

          // Constructor: nodes array
          expect(output).toContain('this.__for0_nodes = []');

          // connectedCallback: renderEach method call (Phase 3: via __invalidate, not __effect)
    ...
    fc.assert(
      fc.property(
        ...
        (itemVar, signalName) => {
          ...
          // Reactive binding: static assignment with this._state (Phase 3: all bindings are static in __renderEach_N)
...
describe('codegen each — unit tests', () => {
  ...
  it('generates reactive show binding for component-level expression', () => {
    ...
    const output = generateComponent(pr);
    // Reactive show: static assignment (Phase 3: no __effect, rendered statically in __renderEach_N)
