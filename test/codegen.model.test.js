...
import { generateComponent, pathExpr } from '../lib/codegen.js';
      ...
      fc.property(arbParseResultWithModel, (ir) => {
        ...
        for (const mb of ir.modelBindings) {
          if (mb.prop === 'checked' && mb.radioValue !== null) {
            ...
            expect(afterCC).toContain(`this.${mb.varName}.checked = (this._state.${mb.signal} === '${mb.radioValue}')`);
          } else if (mb.prop === 'checked') {
            // Checkbox: coerce to boolean
            expect(afterCC).toContain(`this.${mb.varName}.checked = !!this._state.${mb.signal}`);
          } else {
            // Value-based
            expect(afterCC).toContain(`this.${mb.varName}.value = this._state.${mb.signal} ?? ''`);
          }
        }
      ...
      fc.property(arbParseResultWithModel, (ir) => {
        ...
        for (const mb of ir.modelBindings) {
          ...
          if (mb.prop === 'checked' && mb.radioValue === null) {
            ...
            expect(afterCC).toContain(`this._state.${mb.signal} = e.target.checked`);
          } else if (mb.coerce) {
            // Number: Number(e.target.value)
            expect(afterCC).toContain(`this._state.${mb.signal} = Number(e.target.value)`);
          } else {
            // All others: e.target.value
            expect(afterCC).toContain(`this._state.${mb.signal} = e.target.value`);
          }
        }
      }),
      { numRuns: 100 }
...
describe('generateComponent — model edge cases', () => {
  ...
  it('generates radio checked comparison with radioValue', () => {
    ...
    expect(output).toContain("this.__model0.checked = (this._state.color === 'red')");
    expect(output).toContain("this.__model0.addEventListener('change'");
    expect(output).toContain("this._state.color = e.target.value");
  });
  ...
  it('generates multiple model effects in document order', () => {
    ...
    const effect0 = output.indexOf("this.__model0.value = this._state.name");
    const effect1 = output.indexOf("this.__model1.checked = !!this._state.agreed");
    const effect2 = output.indexOf("this.__model2.value = this._state.age");
  ...
  it('generates model effects alongside other effects', () => {
    ...
    expect(output).toContain("this.__b0.textContent = this._state.name");
    expect(output).toContain("this.__model0.value = this._state.name");
