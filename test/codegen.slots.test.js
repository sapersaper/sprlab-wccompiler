...
describe('codegen slots — unit tests', () => {
  ...
  // Phase 4: reactive effect removed
    expect(output).not.toContain('if (this.__slotTpl_data)');
    expect(output).not.toContain('item: this._state.currentItem');
    expect(output).not.toContain('this.__s0.innerHTML = __html');
  });
  ...
  it('scoped slot with multiple props — generates all prop references', () => {
    ...
    // Phase 4: scoped slot reactivities removed
    expect(output).not.toContain('item: this._state.currentItem');
    expect(output).not.toContain('index: this._state.currentIndex');
  });

  it('scoped slot with computed source — uses _state. prefix', () => {
    const pr = makeParseResult({
    ...
    // Phase 4: scoped slot reactivities removed
    expect(output).not.toContain('name: this._state.fullName');
  });
  ...
  it('scoped slot with prop source — uses _s_ prefix', () => {
    ...
    // Phase 4: scoped slot reactivities removed
    expect(output).not.toContain('heading: this._state.title');
  });
  ...
  // Phase 4: scoped slot reactivities removed
    expect(output).not.toContain('if (this.__slotTpl_data)');
    expect(output).not.toContain('item: this._state.current');
  });
  ...
  it('slot parser detects slot="name" on regular elements (cross-framework support)', () => {
    const pr = makeParseResult({
      processedTemplate: '<div><span data-slot="header">Default Header</span><span data-slot="default">Body</span></div>',
      slots: [
        { varName: '__s0', name: 'header', path: ['childNodes[0]', 'childNodes[0]'], defaultContent: 'Default Header', slotProps: [] },
        { varName: '__s1', name: '', path: ['childNodes[0]', 'childNodes[1]'], defaultContent: 'Body', slotProps: [] },
      ],
    });
    ...
    // The generated slot parser should check for slot="name" attribute on regular elements
    expect(output).toContain("child.getAttribute('slot')");
    expect(output).toContain("child.removeAttribute('slot')");
    expect(output).toContain('child.outerHTML');
  });
});
