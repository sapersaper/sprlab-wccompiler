...
const arbParseResultWithShow = fc.record({
  ...
  // Assign sequential varNames and ensure expressions reference existing signals
  const showBindings = r.showBindings.map((sb, i) => ({
    ...sb,
    varName: `__show${i}`,
    expression: signals.length > 0 ? signals[i % signals.length].name : sb.expression,
  }));
    ...
    fc.assert(
      fc.property(arbParseResultWithShow, (ir) => {
        ...
        for (const sb of ir.showBindings) {
          ...
          // style.display assignment in __invalidate method
        ...
        // Each ShowBinding is handled by __invalidate
        expect(output).toContain('__invalidate(key)');
      }),
      { numRuns: 100 }
    );
  });
 ...
 * the Code Generator SHALL transform signal x to this._state.x, computed y to
 * this._state.y, and prop z to this._state.z using transformExpr.
 *
...
describe('Feature: show-directive, Property 4: Expression Auto-Unwrap', () => {
  ...
  it('transforms signal to this._state.x, computed to this._state.y, prop to this._state.z', () => {
    fc.assert(
      fc.property(arbNames, ({ signalName, computedName, propName }) => {
        ...
        expect(propResult).toBe(`this._state.${propName}`);
      }),
      { numRuns: 100 }
    );
  });
...
describe('codegen show — unit tests', () => {
  ...
  it('transforms complex expression: count > 0 && isActive', () => {
    ...
    const output = generateComponent(ir);
    expect(output).toContain('this._state.count > 0 && this._state.isActive');
  });
  ...
  it('generates show effects alongside text binding and event effects', () => {
    ...
    // Show effect
    expect(output).toContain('this.__show0.style.display = (this._state.isVisible) ?');
  });
