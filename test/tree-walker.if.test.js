...
import { walkTree, processIfChains, walkBranch } from '../lib/tree-walker.js';
...
describe('Feature: if, Property 1: Chain Detection and If_Block Structure', () => {
  ...
  it('produces one IfBlock per chain with sequential varNames, valid anchorType, and correct branch types', { timeout: 30000 }, () => {
    fc.assert(
      fc.property(templateArb, (chains) => {
        ...
        for (let c = 0; c < chains.length; c++) {
          ...
          expect(block.anchorType).toBe('if');
          expect(block.anchorIndex).toBe(c);
...
describe('processIfChains — unit tests', () => {
  it('produces correct anchorType and anchorIndex after DOM normalization (Req 2.3)', () => {
    ...
    expect(ifBlocks[0].anchorType).toBe('if');
    expect(ifBlocks[0].anchorIndex).toBe(0);
  });
