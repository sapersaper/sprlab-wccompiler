...
describe('walkTree — model:propName detection', () => {
  ...
  it('generates sequential varNames for multiple model:propName bindings', () => {
    ...
    expect(modelPropBindings[0].varName).toBe('__modelProp_value');
    expect(modelPropBindings[1].varName).toBe('__modelProp_count');
  });
  ...
  it('handles model="signal" and model:propName in the same template without conflict', () => {
    ...
    expect(modelBindings.every(b => b.varName.startsWith('__model_'))).toBe(true);
    expect(modelPropBindings.every(b => b.varName.startsWith('__modelProp_'))).toBe(true);
  });
});
