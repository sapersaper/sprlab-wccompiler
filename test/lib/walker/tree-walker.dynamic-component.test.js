...
import { processDynamicComponents } from '../../../lib/tree-walker.js';
...
describe('processDynamicComponents — basic detection', () => {
  it('detects <component :is="currentView()"> and produces a binding', () => {
    ...
    expect(result[0].anchorType).toBe('dynamic');
    expect(result[0].anchorIndex).toBe(0);
  });
...
// ── Anchor type and index assignment ───────────────────────────────

describe('processDynamicComponents — anchor type and index assignment', () => {
  it('assigns correct anchorType and anchorIndex for a top-level <component>', () => {
  ...
  it('assigns correct anchorType and anchorIndex with a parentPath prefix', () => {
    const root = makeRoot('<component :is="view()"></component>');
  ...
  it('assigns correct anchorType and anchorIndex when <component> is nested inside other elements', () => {
  ...
  it('assigns correct anchorType and anchorIndex when <component> has siblings before it', () => {
...
describe('processDynamicComponents — sequential varNames', () => {
  it('produces __dyn0, __dyn1 for two <component> elements', () => {
    ...
    expect(result[1].anchorType).toBe('dynamic');
    expect(result[1].anchorIndex).toBe(1);
  });
