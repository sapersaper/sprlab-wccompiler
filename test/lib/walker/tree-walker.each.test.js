...
import { parseEachExpression, processForBlocks, walkBranch } from '../../../lib/tree-walker.js';
 ...
 * sequential varNames, valid anchorType/anchorIndex, correct metadata, and
    ...
    fc.assert(
      fc.property(
        ...
        (elements) => {
          ...
          // anchorType and anchorIndex (Phase 5)
          for (let i = 0; i < forBlocks.length; i++) {
            const fb = forBlocks[i];
            expect(fb.anchorType).toBe('each');
            expect(fb.anchorIndex).toBe(i);
          }
...
describe('tree-walker each — unit tests', () => {
  it('produces correct anchorType and anchorIndex after element replacement (Req 3.3)', () => {
    const root = makeRoot('text before<li each="item in items">{{item}}</li>text after');
    const forBlocks = processForBlocks(root, [], new Set(), new Set(), new Set());

    expect(forBlocks).toHaveLength(1);
    expect(forBlocks[0].anchorType).toBe('each');
    expect(forBlocks[0].anchorIndex).toBe(0);
    // The comment should be in the DOM
    let node = root.firstChild;
    let foundComment = false;
    while (node) {
      if (node.nodeType === 8 && node.textContent.trim() === 'each') {
        foundComment = true;
        break;
      }
