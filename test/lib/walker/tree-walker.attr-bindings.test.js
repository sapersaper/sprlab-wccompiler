    ...
    fc.assert(
      fc.property(templateArb, (elements) => {
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          // varName uses __attr_ prefix with sanitized attr name and index
          // Hyphens in attr names are replaced with underscores in varNames
          const sanitizedAttr = el.attr.replace(/-/g, '_');
          expect(ab.varName).toBe(`__attr_${sanitizedAttr}_${i}`);
