...
import { extractDeps, refsComputedOrMethod, buildDepGraph, extractComputedDeps, topologicalSortComputeds } from '../../../lib/codegen.js';
...
describe('buildDepGraph', () => {
  ...
  it('adds constant function bindings to depGraph (Phase 4: no effectBindings)', () => {
    const parseResult = {
    ...
    expect(depGraph.size).toBe(1);
    expect(depGraph.get('count')).toEqual([
      { type: 'text', varName: '__b0', expr: 'this._const_formatter()' },
    ]);
    expect(effectBindings.text).toHaveLength(0);
  });
