...
/** Strip ES module export statements so code can run in a classic <script> tag (jsdom) */
function stripExports(code) {
  return code.replace(/^export default .+;?\s*$/gm, '');
}
// ── Helpers ─────────────────────────────────────────────────────────
...
describe('Constructor spec compliance — Bug Condition', () => {
  it('document.createElement should produce an element without triggering NotSupportedError', async () => {
    ...
    try {
      ...
      scriptEl.textContent = stripExports(code);
  ...
  it('generated code contains signal initialization for each signal', () => {
    fc.assert(
      fc.property(
        ...
        (ir) => {
          const output = generateComponent(ir);
          // Signals are now initialized in the Proxy initial object
          expect(output).toContain('this._state = new Proxy(');
          for (const s of ir.signals) {
            expect(output).toContain(`${s.name}:`);
          }
        }
      ),
      { numRuns: 100 }
  ...
  it('generated code contains prop signal initialization for each prop', () => {
    fc.assert(
      fc.property(
        ...
        (ir) => {
          const output = generateComponent(ir);
          // Props are now initialized in the Proxy initial object
          expect(output).toContain('this._state = new Proxy(');
