...
describe('transformMethodBody — emit call transformation', () => {
  it('transforms emit calls to this._emit calls', () => {
    ...
    expect(result).toBe("this._emit('change', this._state.count)");
  });
  ...
  it('transforms emit calls in effect bodies', () => {
    ...
    expect(result).toBe("this._emit('update', this._state.value)");
  });
});
...
describe('transformMethodBody — emitsObjectName exclusion', () => {
  it('excludes emitsObjectName from signal transforms', () => {
    ...
    // Should be transformed to this._emit('change'), NOT this._state.emit('change')
    expect(result).toBe("this._emit('change')");
    expect(result).not.toContain('this._state.emit');
  });
...
describe('transformExpr — emitsObjectName exclusion', () => {
  ...
  it('excludes emitsObjectName from computed transforms in expressions', () => {
    ...
    expect(result).not.toContain('this._state.emit');
    expect(result).toContain('this._state.doubled');
  });
});
...
describe('generateComponent — emit call transformation in methods', () => {
  it('transforms emit calls in generated method bodies', () => {
    ...
    const output = generateComponent(ir);
    expect(output).toContain("this._emit('change', this._state.count)");
  });

  it('emit calls in effects are not generated (effect removed in Phase 4)', () => {
    const ir = makeIR({
      emits: ['update'],
      emitsObjectName: 'emit',
    ...
    const output = generateComponent(ir);
    // Phase 4: effects removed, emit calls in effects never reach codegen
    expect(output).not.toContain("this._emit('update', this._state.value)");
  });
  ...
  it('does not treat emitsObjectName as a signal in generated output', () => {
    ...
    const output = generateComponent(ir);
    // Should NOT contain this._state.emit as a signal read pattern
    ...
    fc.assert(
      fc.property(
        ...
        ([emitsObjName, signalNames]) => {
          ...
          expect(result).not.toContain(`this._state.${emitsObjName}`);

          // Signal reads should be transformed
          for (const s of signalNames) {
            expect(result).toContain(`this._state.${s}`);
          }
        }
      ),
      { numRuns: 100 }
