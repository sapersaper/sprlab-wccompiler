 ...
 * Verifies the generated code has slot resolution in __renderEach_N
 * and scoped slot template storage in connectedCallback.
 */
...
describe('BUG-0012: scoped slots in each loops', () => {
  it('generates slot resolution in __renderEach_N', () => {
    const result = generateComponent(makeParseResult({
      ...
      forBlocks: [{
        ...
        anchorType: 'each',
        anchorIndex: 0,
        bindings: [{ varName: '__b0', name: 'item', type: 'signal', path: ['childNodes[0]'] }],
        events: [],
        showBindings: [],
        attrBindings: [],
        modelBindings: [],
        slots: [{
          ...
          defaultContent: '{{item}}',
          slotProps: [
            { prop: 'item', source: 'item' },
          ],
        }],
      }],
    }));

    // Should have scoped slot resolution in the render method
    expect(result).toContain('__slotTpl_item');
    expect(result).toContain("replace('{%item%}'");
    expect(result).toContain("querySelector('[data-slot=\"item\"]')");
    expect(result).toContain('__wrap.innerHTML = __slotHtml');
  });
    ...
    const result = generateComponent(makeParseResult({
      ...
      forBlocks: [{
        ...
        slots: [{
          ...
          path: ['childNodes[0]'],
          defaultContent: '{{item}}',
          slotProps: [{ prop: 'item', source: 'item' }],
        }],
      }],
    }));

    // The connectedCallback should store scoped slot templates from __slotMap
  ...
  it('handles each with index variable in scoped slot', () => {
    const result = generateComponent(makeParseResult({
      tagName: 'wcc-list-idx',
      className: 'WccListIdx',
      signals: [{ name: 'items', value: "['a','b']" }],
      processedTemplate: '<ul><!-- each --></ul>',
      forBlocks: [{
        varName: '__for0',
  ...
  it('renders nested each with scoped slots correctly', () => {
    ...
    // Nested each should also have slot resolution
