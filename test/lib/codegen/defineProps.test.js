...
import { camelToKebab } from '../../../lib/parser.js';
...
describe('generateComponent — prop signal initialization', () => {
  ...
  it('places prop signals before user signals', () => {
    ...
    const output = generateComponent(ir);
    const propIdx = output.indexOf("label:");
    const sigIdx = output.indexOf('count:');
...
describe('generateComponent — getters and setters', () => {
  it('generates getter and setter for each prop', () => {
    ...
    expect(output).toContain('get itemCount() { return this._state.itemCount; }');
    expect(output).toContain("set itemCount(val) { this._state.itemCount = val; this.setAttribute('item-count', String(val)); }");
  });
});
...
describe('generateComponent — prop binding effects', () => {
  it('generates effect with _s_ prefix for prop bindings', () => {
    ...
    const output = generateComponent(ir);
    expect(output).toContain("this.__b0.textContent = this._state.label ?? ''");
  });
});

describe('transformExpr — props.x transformation', () => {
  it('transforms props.propName to this._state.propName', () => {
    const result = transformExpr('props.label + " world"', [], [], 'props', new Set(['label']));
    expect(result).toBe('this._state.label + " world"');
  });
...
describe('transformMethodBody — props.x transformation', () => {
  it('transforms props.propName in method bodies', () => {
    ...
    expect(result).toBe('console.log(this._state.label)');
  });
...
describe('generateComponent — props.x in method bodies', () => {
  it('transforms props.label to this._state.label in generated methods', () => {
    const ir = makeIR({
      propDefs: [{ name: 'label', default: "'Click'", attrName: 'label' }],
      propsObjectName: 'props',
    ...
    const output = generateComponent(ir);
    expect(output).toContain('console.log(this._state.label)');
  });
});
...
describe('Feature: define-props, Property 4: Codegen Prop Signal Initialization', () => {
  it('constructor contains prop initialization in _state Proxy for each prop', () => {
    fc.assert(
      fc.property(
        fc.uniqueArray(arbPropName, { minLength: 1, maxLength: 10 }).chain(names =>
          fc.tuple(
        ...
        ([propNames, defaults]) => {
          ...
          // The new format puts props in the Proxy initial object
          expect(output).toContain('this._state = new Proxy(');
          for (let i = 0; i < propNames.length; i++) {
            // Props appear as key: value in the Proxy initial object
            expect(output).toContain(`${propNames[i]}:`);
          }
        }
      ),
      { numRuns: 100 }
 ...
 * SHALL replace every props.propName with this._state.propName and leave
...
describe('Feature: define-props, Property 5: Props Access Transformation Correctness', () => {
  it('transforms props.propName to this._state.propName and leaves non-props unchanged', () => {
    fc.assert(
      fc.property(
        ...
        (propNames) => {
          ...
          for (const name of propNames) {
            expect(result).toContain(`this._state.${name}`);
