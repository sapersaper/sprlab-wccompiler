...
import { generateComponent, transformExpr, pathExpr } from '../../../lib/codegen.js';
import { BOOLEAN_ATTRIBUTES } from '../../../lib/types.js';
...
describe('Feature: attr-bindings, Property 4: Codegen Regular Attribute Effect', () => {
  ...
  const arbIR = fc.record({
    ...
    // Ensure each attr binding expression references an existing signal
    const attrBindings = r.attrBindings.map((ab, i) => ({
      varName: `__attr${i}`,
      attr: ab.attr,
      expression: signals.length > 0 ? signals[i % signals.length].name : ab.expression,
      kind: 'attr',
      path: ab.path,
    }));
...
describe('Feature: attr-bindings, Property 5: Codegen Boolean Attribute Effect', () => {
  ...
  const arbIR = fc.record({
    ...
    // Add signals matching the expressions so they are treated as reactive
    const signalNames = new Set(attrBindings.map(ab => ab.expression));
    const signals = [...signalNames].map(name => ({ name, value: 'false' }));
...
describe('Feature: attr-bindings, Property 6: Codegen Class Binding Effect', () => {
  ...
  const arbIR = fc.record({
    ...
    // Extract signal names from expressions
    const signalNames = new Set();
    for (const cb of r.classBindings) {
      if (cb.isObject) {
        // Extract identifiers from object values like { active: isActive }
        const matches = cb.expression.match(/:\s*([a-zA-Z_]\w*)/g) || [];
        for (const m of matches) {
          signalNames.add(m.replace(/^:\s*/, ''));
        }
      } else {
        signalNames.add(cb.expression);
      }
    }
    const signals = [...signalNames].map(name => ({ name, value: 'false' }));
...
describe('Feature: attr-bindings, Property 7: Codegen Style Binding Effect', () => {
  ...
  const arbIR = fc.record({
    ...
    // Extract signal names from expressions
    const signalNames = new Set();
    for (const sb of r.styleBindings) {
      if (sb.isObject) {
        // Extract identifiers from object values like { color: textColor }
        const matches = sb.expression.match(/:\s*([a-zA-Z_]\w*)/g) || [];
        for (const m of matches) {
          signalNames.add(m.replace(/^:\s*/, ''));
        }
      } else {
        signalNames.add(sb.expression);
      }
    }
    const signals = [...signalNames].map(name => ({ name, value: "''" }));
...
describe('codegen attr-bindings — unit tests', () => {
  ...
  it('transforms complex expression with signal references', () => {
    ...
    const output = generateComponent(ir);
    expect(output).toContain("this._state.base + '/' + this._state.path");
  });
  ...
  it('generates multiple attr effects in document order', () => {
    ...
    ], [{ name: 'url', value: "''" }, { name: 'isOff', value: 'false' }, { name: 'cls', value: "''" }]);
