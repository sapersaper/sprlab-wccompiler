...
import { walkBranch } from '../../../lib/tree-walker.js';
...
describe('4.1 nested each inside each — walkBranch returns forBlocks', () => {
  it('walkBranch detects nested each directive and returns non-empty forBlocks array', () => {
    ...
    expect(nestedFor.anchorType).toBe('each');
    expect(nestedFor.anchorIndex).toBe(0);
  });
...
describe('4.2 if/else inside each — walkBranch returns ifBlocks', () => {
  it('walkBranch detects if/else chain and returns non-empty ifBlocks array', () => {
    ...
    expect(ifBlock.anchorType).toBe('if');
    expect(ifBlock.anchorIndex).toBe(0);
  });
...
describe('4.3 nested each codegen — generates nested forEach with proper variable scoping', () => {
  it('generates nested forEach when forBlock has nested forBlocks', () => {
    const parseResult = {
      ...
      forBlocks: [{
        ...
        anchorType: 'each', anchorIndex: 0, anchorPath: ['childNodes[0]'],
        bindings: [],
        events: [],
        showBindings: [],
        attrBindings: [],
        ...
        forBlocks: [{
          ...
          anchorType: 'each', anchorIndex: 1, anchorPath: ['childNodes[0]'],
    ...
    // Should contain the inner template creation (Phase 3: inline template creation)
    expect(output).toContain("clone.innerHTML = `<span></span>`");

    // Should contain the inner anchor reference
    expect(output).toContain('__anchor___for1');
  ...
  it('generates proper variable scoping — outer var accessible in inner scope', () => {
    ...
    // cat.name should NOT be transformed to this._state.cat since it's a loop variable
    // (note: this._categories is fine — we check specifically for this._state.cat with parens)
...
describe('4.4 if/else inside each codegen — generates per-item conditional logic', () => {
  it('generates per-item conditional when forBlock has nested ifBlocks', () => {
    const parseResult = {
      ...
      forBlocks: [{
        ...
        ifBlocks: [{
          varName: '__if0',
          anchorType: 'if', anchorIndex: 0, anchorPath: ['childNodes[0]'],
          branches: [
    ...
    // Should contain branch selection logic (Phase 3: uses _active instead of _branch)
    expect(output).toContain('__if0_active');
  ...
  it('generates else branch logic using item variable', () => {
    ...
    // Phase 3: else branch sets active = 2 (on separate line from else {)
    expect(output).toContain('__if0_active = 2');
...
describe('5.5 Preservation — single-level each with bindings/events/show/attr/model produces correct output', () => {
  ...
  it('single-level each with model binding produces correct output without nested logic', () => {
    ...
    expect(output).toContain("this._state.selected");
    expect(output).toContain("addEventListener('input'");
    expect(output).toContain("this._state.selected = e.target.value");
  ...
  it('single-level each with ALL directive types combined produces correct output without nested logic', () => {
    ...
    expect(output).toContain("this._state.query");
    expect(output).toContain("addEventListener('input'");
    ...
    // Should contain the standard each structure (Phase 3: __renderEach_0, not __effect)
    ...
    expect(output).toContain('__renderEach_0()');
