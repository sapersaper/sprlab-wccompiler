...
<p>{{ username() }}</p>
      ...
      expect(code).toContain('return this._state.username');
...
<input model="agree" type="checkbox" />
      ...
      expect(code).toContain('return this._state.username');
      expect(code).toContain('return this._state.age');
      expect(code).toContain('return this._state.agree');
...
<input model="value" />
      ...
      // Verify getter branch returns state value
      expect(code).toMatch(/return\s+this\._state\.value/);
...
<input model="name" />
      ...
      // Model binding reads from state
      expect(code).toContain('this.__model_name_0.value = this._state.name');
      expect(code).toContain("addEventListener('input'");
      // Model binding calls _modelSet_ (through wrapper) to emit events
      expect(code).toContain('this._modelSet_name(e.target.value');
...
<input model="email" type="email" />
      ...
      // Verify model binding reads from state
      expect(code).toContain('this.__model_email_0.value = this._state.email');
      
      // Verify event listener calls _modelSet_ to emit events
      expect(code).toContain("addEventListener('input'");
      expect(code).toContain('this._modelSet_email(e.target.value');
...
<input model="enabled" type="checkbox" />
      ...
      // Verify checkbox binding reads from state
      expect(code).toContain('this.__model_enabled_0.checked = !!this._state.enabled');
      
      // Verify change event listener calls _modelSet_ to emit events
      expect(code).toContain("addEventListener('change'");
      expect(code).toContain('this._modelSet_enabled(e.target.checked');
...
<p>{{ country() }}</p>
      ...
      // Verify state is used in model binding
      expect(code).toContain('this._state.country');
...
<p>{{ bio() }}</p>
      ...
      // Verify state is used in model binding
      expect(code).toContain('this._state.bio');
