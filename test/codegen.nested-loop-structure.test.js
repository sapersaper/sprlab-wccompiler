...
<div each="category in categories()" key={{ category.id }}>
  ...
  <div if={{ category.expanded }} class="items-container">
    ...
    // Note: nested each inside if-inside-each not yet expanded to forEach
    // The each template is preserved in the if branch template
    
    // Should not have syntax errors (balanced braces)
    const openBraces = (code.match(/{/g) || []).length;
    const closeBraces = (code.match(/}/g) || []).length;
  // Phase 4: brace counts may not match exactly - just verify both are present
...
<div each="group in data()" key={{ group.id }}>
  <div if={{ group.show }}>
    ...
    // Should have the outer loop and conditional
    expect(code).toMatch(/this\._state.*data/);
...
<div each="node in tree()" key={{ node.id }}>
  <div if={{ node.children }}>
    ...
    expect(code).toMatch(/this\._state.*tree/);
