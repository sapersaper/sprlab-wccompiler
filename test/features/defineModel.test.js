...
<div>{{value()}}</div>
    ...
    // 7. Template effects use _m_ prefix (bug fix: was using this._state.value instead of this._state.value)
...
<p>{{count()}}</p>
    ...
    expect(code).toContain('this._state.modelValue');
...
<span>{{modelValue()}}</span>
    ...
    expect(code).toContain('this._state.localInput = e.target.value');
    expect(code).toContain('this.__model_localInput_0');
    ...
    // No interference: form model uses _state.localInput, defineModel uses _state.modelValue
    // Both should be present without conflict
    expect(code).toContain('this._state.localInput');
    expect(code).toContain('this._state.modelValue');
  });
...
<span>{{enabled()}}</span>
    ...
    expect(code).toContain('this._state.agreed = e.target.checked');
...
<wcc-child model:value="searchText"></wcc-child>
    ...
    expect(code).toContain('this._state.searchText');
    ...
    expect(code).toContain('this._state.searchText = e.detail.value');
