/**
 * Property-based test for SFC compilation.
 *
...
// ── Property: SFC compilation produces valid output ─────────────────

/**
 * Validates: Requirements 4.1, 4.3
 *
 * For all components defined in SFC format, compile() SHALL produce
 * valid JavaScript output containing the expected class and registration.
 */
describe('Property: SFC compilation produces valid output', () => {
  it('compile() produces valid output for SFC format with signal, template, and style', () => {
    fc.assert(
      fc.asyncProperty(
        arbComponent,
        async ([tagName, signalName, signalValue, css]) => {
          ...
          try {
            ...
            // ── Compile ──
            const { code: sfcOutput } = await compile(join(dir, 'comp.wcc'));

            // ── Verify output is valid ──
            expect(sfcOutput).toContain('customElements.define');
            expect(sfcOutput).toContain(tagName);
            expect(sfcOutput).toContain('_state');
          } finally {
            cleanupDir(dir);
          }
        }
...
.demo { display: flex; gap: 8px; }
    ...
    const { code: output } = await compile(join(dir, 'wcc-demo.wcc'));

    // Contains reactive runtime (Phase 2: no __computed or __effect needed for simple components)
...
div { padding: 4px; }
    ...
    const { code: output } = await compile(join(dir, 'wcc-typed.wcc'));
...
<template>
  <span>{{text()}}</span>
    ...
    // Uses explicit import to declare the child component dependency
...
<template>
  <div>
    ...
    // Parent output should contain a named import for the child .wcc resolved as .js
    expect(output).toContain("import WccChild from './wcc-child.js'");
