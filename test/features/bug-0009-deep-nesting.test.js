...
import { extractConstants } from '../../lib/parser-extractors.js';
import { isStaticForExpr } from '../../lib/codegen.js';
...
<div>
  <div each="item in items()" :key="item.id">
    <div if="item.show">
      ...
      // Should contain if block and __renderEach_0
...
<div>
  ...
  // TODO: This test produces invalid JS due to double-transformation bug in transformForExpr
  it('full deep nesting (6 levels) compiles to valid JS', async () => {
    ...
    // Must contain __renderEach_0 for each-loop rendering
    expect(code).toContain('__renderEach_0() {');
