...
.card { padding: 16px; }
  ...
  // Phase 4: scoped slot effect removed
      expect(output).not.toContain('if (this.__slotTpl_info)');

      // Props are resolved from signals
  // Phase 4: scoped slot prop resolution removed
      expect(output).not.toContain('name: this._state.userName');
      expect(output).not.toContain('age: this._state.userAge');

      // Phase 4: scoped slot effect removed — regex, nullish coalescing, global flag no longer generated
      expect(output).not.toContain("(?:\\\\{\\\\{|\\\\{%)\\\\s*");
      expect(output).not.toContain("(?:\\\\}\\\\}|%\\\\})");
      expect(output).not.toContain("v ?? ''");
      expect(output).not.toContain("'g')");
      expect(output).not.toContain('innerHTML = __html');
    } finally {
      cleanupDir(dir);
    }
  });
...
<ul>
  ...
  // Phase 4: Object.entries loop removed
      expect(output).not.toContain('Object.entries(__props)');

      // Phase 4: combined regex pattern removed
      expect(output).not.toContain("(?:\\\\{\\\\{|\\\\{%)\\\\s*");
      expect(output).not.toContain("(?:\\\\}\\\\}|%\\\\})");
    } finally {
      cleanupDir(dir);
...
<table>
  <slot name="cell" :data="rowData" :index="rowIndex" :selected="rowSelected"></slot>
      ...
      // Phase 4: scoped slot prop resolution removed
      expect(output).not.toContain('data: this._state.rowData');
      expect(output).not.toContain('index: this._state.rowIndex');
      expect(output).not.toContain('selected: this._state.rowSelected');

      // Template storage
      expect(output).toContain("this.__slotTpl_cell = __slotMap['cell'].content");

      // Phase 4: combined regex removed
      expect(output).not.toContain('for (const [k, v] of Object.entries(__props))');
      expect(output).not.toContain("(?:\\\\{\\\\{|\\\\{%)\\\\s*");
    } finally {
      cleanupDir(dir);
    }
