...
<div>{{count()}}</div>
    ...
    // Signal initialization in Proxy state
    expect(output).toContain('count:');

    // emit variable is NOT treated as a signal
    expect(output).not.toContain('emit:');
...
<div>{{count()}}</div>
    ...
    expect(output).toContain('_emit(name, detail)');
    expect(output).toContain("this._emit('update', this._state.count)");
  });
...
<div>{{label}}</div>
    ...
    expect(output).toContain('_emit(name, detail)');
    expect(output).toMatch(/this\._emit\(['"]change['"], this\._state\.value\)/);

    // Props access in method
    expect(output).toContain('this._state.label');

    // emit is NOT treated as a signal
