...
<div>
  <div if="{{ items().length > 0 }}">
    ...
    expect(code).toMatch(/this._state.items.length\s*>\s*0/);
    
    // Should NOT have HTML entities like &gt;
    expect(code).not.toContain('&gt;');
    
    // Should NOT have malformed attributes
    expect(code).not.toMatch(/this._state.items.length=""/);
  });
...
<div>
  <div if="{{ count() > min() && count() < max() }}">
    ...
    const hasValidCondition = code.includes('_state.count') && 
                              code.includes('_state.min') && 
                              code.includes('_state.max');
...
<div>
  <div if="{{ status() === 'active' }}">
    ...
    expect(code).toMatch(/this._state.status\s*===\s*['"]active['"]/);
...
<div>
  <div if="{{ count() < 10 }}">
    ...
    expect(code).toMatch(/this._state.count\s*<\s*10/);
...
<div>
  <div if="{{ count() >= 5 }}">
    ...
    expect(code).toMatch(/this._state.count\s*>=\s*5/);
...
<div>
  <div if="{{ count() === 0 || status() === 'inactive' }}">
    ...
    expect(code).toMatch(/this._state.count\s*===\s*0/);
    expect(code).toMatch(/this._state.status\s*===\s*['"]inactive['"]/);
  });
