...
describe('Backward compatibility and coexistence', () => {
  describe('slot-template-* works without WccSlotsDirective', () => {
    it('generated code preserves token replacement as else fallback', () => {
      ...
      // Phase 4: scoped slot effect removed — token replacement else fallback no longer generated
      expect(code).not.toContain('} else {');
      expect(code).not.toContain('__html = __html.replace(');
    });
    ...
    it('token replacement still works when no renderer is registered (else path)', () => {
      ...
      // Phase 4: scoped slot effect removed
      expect(code).not.toContain("if (this.__slotRenderers && this.__slotRenderers['stats'])");
      expect(code).not.toContain('} else {');
  ...
  describe('renderer takes priority over token replacement', () => {
    it('generated code invokes renderer and skips token replacement when registered', () => {
      ...
      // Phase 4: scoped slot effect removed — renderer invocation and token replacement no longer generated
      expect(code).not.toContain("this.__slotRenderers['stats'](__props)");
      expect(code).not.toContain('} else if (this.__slotTpl_');
      expect(code).not.toContain('__html = __html.replace(');
    });
  });
  ...
  describe('wcc:slot-update event is always emitted', () => {
    it('event dispatch occurs regardless of renderer registration', () => {
      ...
      // Phase 4: scoped slot effect removed — event dispatch and renderer check no longer generated
      expect(code).not.toContain("this.dispatchEvent(new CustomEvent('wcc:slot-update'");
      expect(code).not.toContain("if (this.__slotRenderers && this.__slotRenderers['stats'])");
    });
  });
  ...
  describe('mixed usage: ng-template[slot] and slot-template-* coexistence', () => {
    it('multiple independent scoped slots each get their own renderer check', () => {
      ...
      // Phase 4: scoped slot effects removed — renderer checks no longer generated
      expect(code).not.toContain("if (this.__slotRenderers && this.__slotRenderers['stats'])");
      expect(code).not.toContain("if (this.__slotRenderers && this.__slotRenderers['details'])");
      expect(code).not.toContain("this.__s1.innerHTML = __html");
      expect(code).not.toContain("this.__s2.innerHTML = __html");
    });
