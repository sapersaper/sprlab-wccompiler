...
<ul>
  <li each="cat in categories">
    <span each="item in cat.items">{{item}}</span>
      ...
      const forEachLine = lines.findIndex(l => l.includes('__for1_iter.forEach((item'));
      ...
      if (outerForEachLine > -1 && forEachLine > -1) {
        ...
        const strayItemRef = betweenLines.find(l => /\bitem\b/.test(l) && !l.includes('__for1_iter') && !l.includes('cat.items'));
        if (strayItemRef) {
          console.log('\n!!! STRAY ITEM REFERENCE FOUND:', strayItemRef.trim());
        }
      }

      // Bug 2: Check that inner loop sets textContent on nodes (Phase 3: uses 'node' not 'innerNode')
      expect(output).toContain('node');
      expect(output).toContain("item ?? ''");

      // The textContent assignment should be INSIDE the inner forEach
      const innerForEachIdx = output.indexOf('__for1_iter.forEach((item');
