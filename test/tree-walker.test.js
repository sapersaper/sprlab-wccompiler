...
import { walkTree } from '../lib/tree-walker.js';
...
describe('walkTree — text bindings', () => {
  ...
  it('assigns descriptive varNames (__text_{name})', () => {
    const root = makeRoot('<div>{{a}}</div><div>{{b}}</div>');
    const { bindings } = walkTree(root, new Set(['a', 'b']), new Set());

    expect(bindings[0].varName).toBe('__text_a_0');
    expect(bindings[1].varName).toBe('__text_b_1');
  });
...
describe('walkTree — event bindings', () => {
  ...
  it('assigns descriptive varNames (__evt_{event}_{handler})', () => {
    const root = makeRoot(
      '<button @click="a">A</button><button @click="b">B</button>'
    );
    const { events } = walkTree(root, new Set(), new Set());

    expect(events[0].varName).toBe('__evt_click_a_0');
    expect(events[1].varName).toBe('__evt_click_b_1');
  });
  ...
  it('discovers every {{varName}} with correct name, valid path, and correct type', () => {
    fc.assert(
      fc.property(templateArb, (elements) => {
        ...
        // Descriptive varNames — each contains the binding name and is unique
        const varNameSet = new Set();
        for (const binding of bindings) {
          expect(binding.varName).toMatch(/^__text_/);
          expect(varNameSet.has(binding.varName)).toBe(false);
          varNameSet.add(binding.varName);
        }
      }),
      { numRuns: 100 }
    );
  ...
  it('discovers every @event with correct name and handler, and removes all @event attributes', () => {
    fc.assert(
      fc.property(templateArb, (elements) => {
        ...
        // Descriptive varNames — each contains event name and is unique
        const varNameSet = new Set();
        for (const ev of events) {
          expect(ev.varName).toMatch(/^__evt_/);
          expect(varNameSet.has(ev.varName)).toBe(false);
          varNameSet.add(ev.varName);
        }
...
// ── BUG-0008: Constant classification with spaces and parentheses ───

describe('walkTree — BUG-0008: constant binding classification with spaces in interpolation', () => {
  it('classifies {{ totalItems() }} (with spaces) as constant when name is in constantNames', () => {
    const root = makeRoot('<p>{{ totalItems() }}</p>');
    const { bindings } = walkTree(root, new Set(), new Set(), new Set(), new Set(['totalItems']));

    expect(bindings).toHaveLength(1);
    expect(bindings[0].name).toBe('totalItems');
    expect(bindings[0].type).toBe('constant');
  });

  it('classifies {{totalItems()}} (no spaces) as constant', () => {
    const root = makeRoot('<p>{{totalItems()}}</p>');
    const { bindings } = walkTree(root, new Set(), new Set(), new Set(), new Set(['totalItems']));
  ...
  it('classifies {{ activeCount() }} as constant in mixed content', () => {
    const root = makeRoot('<div>Total: {{ activeCount() }} items</div>');
    const { bindings } = walkTree(root, new Set(), new Set(), new Set(), new Set(['activeCount']));

    expect(bindings).toHaveLength(1);
    expect(bindings[0].name).toBe('activeCount');
    expect(bindings[0].type).toBe('constant');
  });

  it('trims name correctly — no leading/trailing spaces in binding name', () => {
    const root = makeRoot('<p>{{   myConst()   }}</p>');
    const { bindings } = walkTree(root, new Set(), new Set(), new Set(), new Set(['myConst']));

    expect(bindings).toHaveLength(1);
    expect(bindings[0].name).toBe('myConst');
    expect(bindings[0].type).toBe('constant');
  });

  it('still classifies signals with spaces correctly', () => {
    const root = makeRoot('<p>{{ count() }}</p>');
    const { bindings } = walkTree(root, new Set(['count']), new Set(), new Set(), new Set());
  ...
  it('still classifies bare constants (no parens) correctly', () => {
    const root = makeRoot('<p>{{ MAX_SIZE }}</p>');
    const { bindings } = walkTree(root, new Set(), new Set(), new Set(), new Set(['MAX_SIZE']));

    expect(bindings).toHaveLength(1);
    expect(bindings[0].name).toBe('MAX_SIZE');
    expect(bindings[0].type).toBe('constant');
  });
});
