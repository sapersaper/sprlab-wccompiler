...
<textarea model="bio"></textarea>
      ...
      expect(output).toContain('this.__model_name_0 = __root.');
      expect(output).toContain('this.__model_agreed_1 = __root.');
      expect(output).toContain('this.__model_color_2 = __root.');
      expect(output).toContain('this.__model_color_3 = __root.');
      expect(output).toContain('this.__model_age_4 = __root.');
      expect(output).toContain('this.__model_category_5 = __root.');
      expect(output).toContain('this.__model_bio_6 = __root.');

      // Signal → DOM effects
      // Text input: value with nullish coalesce
      expect(output).toContain("this.__model_name_0.value = this._state.name ?? ''");
      // Checkbox: !! coercion
      expect(output).toContain('this.__model_agreed_1.checked = !!this._state.agreed');
      // Radio red: compare to 'red'
      expect(output).toContain("this.__model_color_2.checked = (this._state.color === 'red')");
      // Radio blue: compare to 'blue'
      expect(output).toContain("this.__model_color_3.checked = (this._state.color === 'blue')");
      // Number: value with nullish coalesce
      expect(output).toContain("this.__model_age_4.value = this._state.age ?? ''");
      // Select: value with nullish coalesce
      expect(output).toContain("this.__model_category_5.value = this._state.category ?? ''");
      // Textarea: value with nullish coalesce
      expect(output).toContain("this.__model_bio_6.value = this._state.bio ?? ''");

      // DOM → Signal event listeners
      // Text input: input event, e.target.value
      expect(output).toContain("this.__model_name_0.addEventListener('input'");
      expect(output).toContain('this._state.name = e.target.value');
      // Checkbox: change event, e.target.checked
      expect(output).toContain("this.__model_agreed_1.addEventListener('change'");
      expect(output).toContain('this._state.agreed = e.target.checked');
      // Radio: change event, e.target.value
      expect(output).toContain("this.__model_color_2.addEventListener('change'");
      // Number: input event, Number(e.target.value)
      expect(output).toContain("this.__model_age_4.addEventListener('input'");
      expect(output).toContain('this._state.age = Number(e.target.value)');
      // Select: change event, e.target.value
      expect(output).toContain("this.__model_category_5.addEventListener('change'");
      expect(output).toContain('this._state.category = e.target.value');
      // Textarea: input event, e.target.value
      expect(output).toContain("this.__model_bio_6.addEventListener('input'");
      expect(output).toContain('this._state.bio = e.target.value');
