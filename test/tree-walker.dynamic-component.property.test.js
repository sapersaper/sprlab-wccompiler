...
describe('Feature: dynamic-component, Property 3: Binding extraction completeness', () => {
  ...
  it('produces a valid varName and anchorType/anchorIndex for the binding', () => {
    fc.assert(
      fc.property(
        exprValue,
        ...
        (isExpr, props, events) => {
          ...
          // anchorType and anchorIndex should be set correctly
          expect(binding.anchorType).toBe('dynamic');
          expect(binding.anchorIndex).toBe(0);
        }
      ),
      { numRuns: 20 }
    );
