...
- [x] 1. Codegen — Combined regex for `{{prop}}` and `{%prop%}`
  - [x] 1.1 Update the scoped slot regex in `lib/codegen.js`
  ...
  - [x] 1.2 Write unit tests for combined regex
  ...
  - [x] 1.3 Write property test for interpolation equivalence
    - For any prop name and value, both `{{prop}}` and `{%prop%}` produce identical output
    - _Requirements: 5.5_

- [x] 2. Codegen — Slot parser detects `slot-template-name` attributes
  - [x] 2.1 Add `slot-template-<name>` detection in the slot resolution loop
  ...
  - [x] 2.2 Add `slot-props` attribute detection for scoped slot prop names
  ...
  - [x] 2.3 Write unit tests for slot-template-name detection
...
- [x] 3. Vue Plugin — Scoped slot transform
  - [x] 3.1 Extend `wccVuePlugin` pre-transform for scoped slots
  ...
  - [x] 3.2 Write unit tests for Vue scoped slot transform
...
- [x] 4. Integration tests
  - [x] 4.1 Write integration test: WCC-to-WCC scoped slots still work
  ...
  - [x] 4.2 Write integration test: `{%prop%}` syntax works in compiled output
    - Create a component with scoped slots
    - Verify the generated regex matches `{%prop%}` tokens
    - _Requirements: 1.1, 1.2, 1.3_

  - [x] 4.3 Write integration test: `slot-template-name` attribute works
    - Verify the generated slot parser code detects the attribute
    - _Requirements: 3.1, 3.2, 3.3, 3.4_

- [x] 5. Documentation
  - [x] 5.1 Update `docs/QA-defineModel.md` or create `docs/QA-scoped-slots.md`
