...
- [ ] 1. Codegen: Emit `__scopedSlots` static array and instance getter
  - [ ] 1.1 Emit `static __scopedSlots = ['slotName1', 'slotName2', ...]` in the custom element class when scoped slots are present
  ...
  - [ ] 1.2 Emit instance getter `get __scopedSlots() { return this.constructor.__scopedSlots || []; }`
    - Allows the directive to access the list from an element instance
    - _Requirements: 7.1_

- [ ] 2. Codegen: Add `registerSlotRenderer` method and `__slotProps` storage
  - [ ] 2.1 Emit `registerSlotRenderer(slotName, callback)` method in the custom element class when scoped slots are present
  ...
  - [ ] 2.2 Initialize `__slotRenderers` and `__slotProps` in the constructor
    - Add `this.__slotRenderers = {}` and `this.__slotProps = {}` initialization
    - Only emit when the component has scoped slots
    - _Requirements: 7.1, 7.2_

- [ ] 3. Codegen: Modify scoped slot effect to emit event and support renderer delegation
  - [ ] 3.1 Store current props in `this.__slotProps[slotName]` within the effect
    - Before any rendering logic, assign the computed props object to `__slotProps`
    - _Requirements: 7.2, 8.5_

  - [ ] 3.2 Emit `wcc:slot-update` CustomEvent with `{ slot, props }` detail and `bubbles: false`
    - Dispatch the event after storing props but before token replacement
    - _Requirements: 8.1, 8.2, 8.3, 8.4_

  - [ ] 3.3 Add conditional check: if a renderer is registered, invoke it and skip token replacement
  ...
  - [ ]* 3.4 Write unit tests for codegen changes
...
- [ ] 5. Angular directive: Create `WccSlotDef` auxiliary directive
  - [ ] 5.1 Create `adapters/angular.ts` with the `WccSlotDef` directive
...
- [ ] 6. Angular directive: Implement `WccSlotsDirective` (main)
  - [ ] 6.1 Create the `WccSlotsDirective` scaffold in `adapters/angular.ts`
  ...
  - [ ] 6.2 Implement `classifyAndInitSlots()` — classification using `__scopedSlots`
  ...
  - [ ] 6.3 Implement `initNamedSlot(slotDef)` — immediate static rendering
  ...
  - [ ] 6.4 Implement `initScopedSlot(slotDef)` — async registration with renderer
  ...
  - [ ] 6.5 Implement `buildContext(props)` — context construction
  ...
  - [ ] 6.6 Implement `renderSlot(slotName, props)` — view creation/update
  ...
  - [ ] 6.7 Implement `insertView(slotName, state)` — DOM insertion for scoped slots
  ...
  - [ ] 6.8 Implement `cleanup()` — lifecycle teardown in `ngOnDestroy`
  ...
  - [ ]* 6.9 Write property tests for `buildContext`
  ...
  - [ ]* 6.10 Write property tests for slot classification
  ...
  - [ ]* 6.11 Write unit tests for directive behavior
...
- [ ] 8. Backward compatibility and coexistence
  - [ ] 8.1 Ensure `slot-template-*` attribute pattern continues to work without the directive
  ...
  - [ ] 8.2 Handle priority when both `ng-template[slot]` and `slot-template-*` target the same slot
  ...
  - [ ]* 8.3 Write integration tests for coexistence scenarios
...
- [ ] 9. Update Angular framework testing app
  - [ ] 9.1 Add a scoped slot demo to `framework-testing/angular/src/app/app.component.ts`
  ...
  - [ ] 9.2 Add or update a WCC component in `framework-testing/angular/src/wcc/` with scoped slots
...
- [ ] 10. Documentation update
  - [ ] 10.1 Update `adapters/angular.js` documentation comments to reference the new directives
  ...
  - [ ] 10.2 Update `FEATURES.md` to document Angular native scoped slots
