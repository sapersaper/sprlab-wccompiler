...
function generateFrameworkStubs(outputDir, config) {
  ...
  // ── React stubs ──
  if (config.integrations.includes('react')) {
  ...
  // ── Vue stubs ──
  if (config.integrations.includes('vue')) {
...
function copyIntegrationPlugin(integration, rootDir, outputDir) {
  const INTEGRATIONS = {
    ...
    angular: [
      { src: join(rootDir, 'integrations/angular-adapter.js'), dest: 'angular-adapter.js' },
      { src: join(rootDir, 'integrations/angular-adapter.d.ts'), dest: 'angular-adapter.d.ts' },
    ],
  };
  ...
  if (command === 'build') {
    ...
    if (config.integrations.length > 0) {
      ...
      for (const integration of config.integrations) {
        copyIntegrationPlugin(integration, rootDir, outputDir);
      }
    }
  } else if (command === 'dev') {
