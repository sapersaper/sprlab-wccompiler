...
const __t_TestBug0019 = document.createElement('template');
__t_TestBug0019.innerHTML = `
<!-- each -->
`;

class TestBug0019 extends HTMLElement {
  static __meta = { tag: 'test-bug0019', props: [], events: [], models: [], slots: [] };
  ...
  connectedCallback() {
    ...
    const __root = __t_TestBug0019.content.cloneNode(true);
    ...
    this.__disposers.push(__effect(() => {
      ...
      __iter.forEach((category, __idx) => {
        ...
        if (__oldMap.has(__key)) {
          ...
          __if0_t0.innerHTML = `<div class="items-container">
    <!-- each -->
  </div>`;
...
if (!customElements.get('test-bug0019')) customElements.define('test-bug0019', TestBug0019);

export default TestBug0019;