...
describe('compile() — attr-bindings integration', () => {
  ...
  <p :style="styleStr">Inline style</p>
      ...
      writeFileSync(
        join(dir, 'attr-test.js'),
        `import { defineComponent, signal } from 'wcc'

export default defineComponent({
  tag: 'wcc-attr-test',
  template: './attr-test.html',
})
...
`
      ...
      const output = await compile(join(dir, 'attr-test.js'));
  ...
  it('bind:attr form produces same output patterns as :attr form', async () => {
    ...
    try {
      writeFileSync(
        join(dir, 'bind-form.html'),
        '<a bind:href="url" bind:disabled="isOff">link</a>'
      );

      writeFileSync(
        join(dir, 'bind-form.js'),
        `import { defineComponent, signal } from 'wcc'

export default defineComponent({
  tag: 'wcc-bind-form',
  template: './bind-form.html',
})
...
`
      ...
      const output = await compile(join(dir, 'bind-form.js'));
