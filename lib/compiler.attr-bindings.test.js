...
<div>
  ...
  <p :style="styleStr">Inline style</p>
      ...
      expect(output).toContain('this.__attr_href_0 =');
      expect(output).toContain('this.__attr_disabled_1 =');
      expect(output).toContain('this.__attr_class_2 =');
      expect(output).toContain('this.__attr_class_3 =');
      expect(output).toContain('this.__attr_style_4 =');
      expect(output).toContain('this.__attr_style_5 =');

      // 3. DOM refs assigned before appendChild (both now in connectedCallback)
      ...
      for (const varName of ['__attr_href_0', '__attr_disabled_1', '__attr_class_2', '__attr_class_3', '__attr_style_4', '__attr_style_5']) {
        const refPos = ccSection.indexOf(`this.${varName} =`);
      ...
      expect(output).toContain("this.__attr_href_0.setAttribute('href'");
      expect(output).toContain("this.__attr_href_0.removeAttribute('href')");

      // 5. Boolean attr effect: :disabled="isLoading" → property assignment with !!
      expect(output).toContain('this.__attr_disabled_1.disabled = !!(');

      // 6. Class binding (object): :class="{ active: isActive, bold: isBold }"
      expect(output).toContain('this.__attr_class_2.classList.add(__k)');
      expect(output).toContain('this.__attr_class_2.classList.remove(__k)');

      // 7. Class binding (string): :class="className"
      expect(output).toContain('this.__attr_class_3.className =');

      // 8. Style binding (object): :style="{ color: textColor, fontSize: size }"
      expect(output).toContain('this.__attr_style_4.style[__k] = __val');

      // 9. Style binding (string): :style="styleStr"
      expect(output).toContain('this.__attr_style_5.style.cssText =');

      // 10. Signal transformation in expressions
      expect(output).toContain('this._state.url');
      expect(output).toContain('this._state.isLoading');
      expect(output).toContain('this._state.isActive');
      expect(output).toContain('this._state.className');
      expect(output).toContain('this._state.textColor');
      expect(output).toContain('this._state.styleStr');

      // 11. Text binding for {{linkText()}}
      expect(output).toContain('this._state.linkText');
...
<a bind:href="url" bind:disabled="isOff">link</a>
      ...
      expect(output).toContain('this._state.url');
      expect(output).toContain('this._state.isOff');
