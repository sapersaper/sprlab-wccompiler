...
describe('Feature: show-directive, Property 3: Codegen Effect Structure', () => {
  it('generates DOM refs in constructor and __effect with style.display in connectedCallback per ShowBinding', () => {
    fc.assert(
      fc.property(arbParseResultWithShow, (ir) => {
        const output = generateComponent(ir);

        for (const sb of ir.showBindings) {
          // DOM element reference in constructor
        ...
        for (const sb of ir.showBindings) {
          const refPos = constructorSection.indexOf(`this.${sb.varName} = __root`);
