...
<div class="container">
  <component :is="currentView()"></component>
      ...
      expect(code).toContain('this._state.currentView');
...
<div>
  ...
  // Phase 4: __effect removed
      ...
      expect(code).toContain('this._state.pageTitle');
      expect(code).toContain('this._state.items');
...
<div>
  ...
  // Phase 4: generated code may not be parseable as a standalone Function due to missing runtime
...
<div>
  <div if="showDynamic()">
    <component :is="currentView()"></component>
      ...
      // The <component> inside the if branch is now properly processed as a
      // dynamic component (replaced with <!-- dynamic --> comment anchor).
      // The branch template should contain the comment anchor.
      expect(code).toContain('<!-- dynamic -->');

      // The dynamic component is created with document.createElement inside the branch setup
      expect(code).toContain('__dyn0_anchor');
      expect(code).toContain('document.createElement(__tag)');
      expect(code).toContain('customElements.upgrade');
      expect(code).toContain('this._state.currentView');
...
<div>
  <div each="tab in tabs()">
    <component :is="tab.tag"></component>
      ...
      // The <component> inside the each loop is now properly processed as a
      // dynamic component (replaced with <!-- dynamic --> comment anchor).
      expect(code).toContain('<!-- dynamic -->');

      // Note: dynamic components inside each loops not yet fully expanded
      // The <!-- dynamic --> placeholder is preserved in the each template
...
<div>
  ...
  <component :is="footer()"></component>
      ...
      expect(code).toContain('this._state.header');
      expect(code).toContain('this._state.content');
      expect(code).toContain('this._state.footer');
    } finally {
      cleanupDir(dir);
    }
  });
