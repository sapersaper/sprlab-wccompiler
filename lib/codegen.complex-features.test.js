...
<div>
  <component each="item in items()" :is="item.type" :key="item.id"></component>
    ...
    // Should have __renderEach_0 method
...
<div>
  <div each="item in items()" :key="item.id">
    ...
    // 3. __renderEach_0 method
    expect(code).toContain('__renderEach_0() {');
    
    // 4. Named slots (content inside <component> is discarded when component is replaced with anchor)
    // Slot content is passed to the dynamic component at runtime, not compiled into the parent.
    // The dynamic component handles its own slot distribution.
    
    // 5. Conditionals (the if block is inside <component> content, so it's discarded)
    // Only top-level conditionals outside <component> would appear
    
    // 6-8. Class/style/event bindings inside <component> are discarded
    // They would be handled by the dynamic component itself
    
    // The dynamic component creation is present
