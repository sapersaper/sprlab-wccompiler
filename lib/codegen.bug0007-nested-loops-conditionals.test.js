...
describe('BUG-0007: Nested loops with conditionals', () => {
  ...
  describe('tree-walker: buildIfBlock captures branch forBlocks', () => {
    ...
    it('should have correct anchor path relative to branch node', () => {
      ...
      // Anchor path should be relative to the branch element (the if div)
      // It should be a short path like ["childNodes[N]"] not a deep nested path
      expect(innerFor.anchorPath.length).toBeLessThanOrEqual(2);
    });
...
<div each="item in items()" :key="item.id" class="item" :class="{ active: item.active, disabled: !item.active }">
  {{ item.name }}
      ...
      // Should use classList.add/remove, NOT setAttribute('class', obj)
      expect(code).toMatch(/classList\.add/);
      expect(code).toMatch(/classList\.remove/);
...
<div each="cat in categories()" :key="cat.id">
  <div if="cat.expanded">
    <div each="item in cat.items" :key="item.id" class="row" :class="{ selected: item.selected }">
      ...
      // Nested items should also use classList
      expect(code).toMatch(/classList\.add/);
      expect(code).toMatch(/classList\.remove/);
    });
...
<div each="item in items()" :key="item.id" :class="item.active ? 'active' : 'inactive'">
  content
      ...
      // Ternary :class should use className assignment
...
<div if="visible()">
  <ul>
    <li each="item in items()" :key="item">{{ item }}</li>
      ...
      expect(code).toMatch(/this\._items\(\)/);

      // Should have __effect inside setup for the each loop
      expect(code).toMatch(/this\.__disposers\.push\(__effect/);
...
<div if="show()">
  <div each="todo in todos()" :key="todo.id" class="todo">
    {{ todo.text }}
      ...
      expect(code).toMatch(/this\._todos\(\)/);
    });
...
<div if="active()">
  <span each="n in numbers()">{{ n }}</span>
      ...
      expect(code).toMatch(/this\._numbers\(\)/);
...
<div each="cat in categories()" :key="cat.id">
  ...
  <div if="cat.expanded" class="items">
    <div each="item in cat.items" :key="item.id">
      ...
      // Should have the items-container class
      expect(code).toMatch(/class="items"/);

      // Should reference cat.items as the inner loop source
      expect(code).toMatch(/cat\.items/);

      // Should have cat.expanded conditional check
      expect(code).toMatch(/cat\.expanded/);

      // The inner anchor should be relative to __if0_node (short path)
      // NOT a deep path like __if0_node.childNodes[7].childNodes[1]
      // It should be something like __if0_node.childNodes[1]
      expect(code).toMatch(/__if0_node\.childNodes\[\d\]/);
      expect(code).not.toMatch(/__if0_node\.childNodes\[\d+\]\.childNodes\[\d+\]\.childNodes/);

      // Should be valid JS (balanced braces)
      const openBraces = (code.match(/{/g) || []).length;
      const closeBraces = (code.match(/}/g) || []).length;
      expect(openBraces).toBe(closeBraces);
    });
...
<div each="g in groups()" :key="g.id">
  <div if="g.open" class="content">
    <span each="c in g.children" :key="c">{{ c }}</span>
      ...
      // Should have branch guard for the forBlock generation
      expect(code).toMatch(/__if0_branch === 0/);

      // Inner loop source should reference g.children
      expect(code).toMatch(/g\.children/);
    });
...
<div each="cat in categories()" :key="cat.id">
  ...
  <div if="cat.expanded" class="items-list">
    <div each="item in cat.items" :key="item">{{ item }}</div>
      ...
      expect(code).toMatch(/class="items-list"/);
      expect(code).toMatch(/cat\.items/);
      expect(code).toMatch(/cat\.expanded/);
