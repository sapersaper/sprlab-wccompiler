...
export async function loadConfig(projectRoot, configFile) {
  ...
  if (typeof config.port !== 'number' || !isFinite(config.port)) {
    const error = new Error(`Error in wcc.config.js: port must be a finite number`);
    error.code = 'INVALID_CONFIG';
    throw error;
  }
  if (typeof config.input !== 'string' || !config.input.trim()) {
    const error = new Error(`Error in wcc.config.js: input must be a non-empty string`);
    error.code = 'INVALID_CONFIG';
    throw error;
  }
  if (typeof config.output !== 'string' || !config.output.trim()) {
    const error = new Error(`Error in wcc.config.js: output must be a non-empty string`);
    error.code = 'INVALID_CONFIG';
    throw error;
  }
  if (typeof config.minify !== 'boolean') {
    const error = new Error(`Error in wcc.config.js: minify must be a boolean`);
    error.code = 'INVALID_CONFIG';
    throw error;
  }
  if (typeof config.comments !== 'boolean') {
    const error = new Error(`Error in wcc.config.js: comments must be a boolean`);
    error.code = 'INVALID_CONFIG';
    throw error;
  }
  if (typeof config.runtime !== 'boolean') {
    const error = new Error(`Error in wcc.config.js: runtime must be a boolean`);
    error.code = 'INVALID_CONFIG';
    throw error;
  }
  if (typeof config.htmlData !== 'boolean') {
    const error = new Error(`Error in wcc.config.js: htmlData must be a boolean`);
    error.code = 'INVALID_CONFIG';
    throw error;
  }
  const validIntegrations = ['vue', 'react', 'angular'];
  if (!Array.isArray(config.integrations)) {
    const error = new Error(`Error in wcc.config.js: integrations must be an array (e.g. ['vue', 'react'])`);
    error.code = 'INVALID_CONFIG';
    throw error;
  }
  for (const i of config.integrations) {
    if (!validIntegrations.includes(i)) {
      const error = new Error(`Error in wcc.config.js: integration '${i}' is not valid. Use: vue, react, angular`);
      error.code = 'INVALID_CONFIG';
      throw error;
    }
  }
