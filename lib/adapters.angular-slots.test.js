...
describe('Backward compatibility and coexistence', () => {
  ...
  describe('renderer takes priority over token replacement', () => {
    it('generated code invokes renderer and skips token replacement when registered', () => {
      ...
      // Token replacement is in the else branch (skipped when renderer exists)
      const rendererCheck = code.indexOf("if (this.__slotRenderers && this.__slotRenderers['stats'])");
      const elseBlock = code.indexOf('} else {', rendererCheck);
      const tokenReplace = code.indexOf('__html = __html.replace(', elseBlock);
      // Token replacement comes AFTER the else (i.e., it's inside the else block)
      expect(rendererCheck).toBeGreaterThan(-1);
      expect(elseBlock).toBeGreaterThan(rendererCheck);
      expect(tokenReplace).toBeGreaterThan(elseBlock);
    });
  });
