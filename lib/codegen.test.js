...
describe('transformMethodBody', () => {
  ...
  it('transforms model signal reads: varName() → this._m_{propName}()', () => {
    ...
    expect(result).toBe('console.log(this._m_value())');
  });
  ...
  it('transforms model signal with different varName and propName', () => {
    ...
    expect(result).toBe('this._modelSet_title(this._m_title() + "!")');
  });
  ...
  it('model transforms do not interfere with regular signal transforms', () => {
    ...
    expect(result).toBe('this._count(this._m_value())');
  });
});
...
describe('transformExpr — model signals', () => {
  it('transforms model signal reads: varName() → this._m_{propName}()', () => {
    ...
    expect(result).toBe('this._m_value() + 1');
  });
  ...
  it('transforms model signal with different varName and propName', () => {
    ...
    expect(result).toBe('this._m_title()');
  });
...
describe('generateComponent — integration', () => {
  it('generates a complete component with all sections', () => {
    ...
    // 1. Reactive runtime (tree-shaken: only __signal and __effect for this component)
  ...
  it('generated output contains reactive runtime, class, connectedCallback, and customElements.define', () => {
    fc.assert(
      fc.property(arbParseResult, (ir) => {
        ...
        // Reactive runtime (always includes __signal; __computed/__effect only if needed)
        expect(output).toContain('function __signal(initial)');
        if (ir.computeds.length > 0) expect(output).toContain('function __computed(fn)');
        const needsEffect = ir.bindings.length > 0 || ir.effects.length > 0 || ir.showBindings?.length > 0 || ir.modelBindings?.length > 0 || ir.attrBindings?.length > 0 || ir.ifBlocks?.length > 0 || ir.forBlocks?.length > 0;
        if (needsEffect) expect(output).toContain('function __effect(fn)');
...
describe('BUG-0008: transformMethodBody transforms constant arrow function bodies', () => {
  it('transforms signal references inside arrow function body', () => {
    ...
    expect(result).toBe('() => this._items().filter(item => item.active).length');
  });
  ...
  it('transforms signal references in simple arrow function', () => {
    ...
    expect(result).toBe('() => this._items().length');
  });
  ...
  it('transforms multiple signal references in arrow function', () => {
    ...
    expect(result).toBe('() => this._items().length + this._count()');
  });
  ...
  it('does not transform the arrow function parameter names', () => {
    ...
    expect(result).toContain('this._items()');
  });
});

describe('BUG-0008: generateComponent handles function constants with reactive effects', () => {
  it('generates __effect for function-type constants in text bindings', () => {
    const ir = {
      tagName: 'wcc-test',
      className: 'WccTest',
    ...
    expect(output).toContain('this._const_totalItems = () => this._items().length');

    // Should call the constant as a function inside an effect
    expect(output).toContain('this.__text_totalItems_0.textContent = this._const_totalItems()');

    // Should be wrapped in __effect (reactive)
  ...
  it('transforms constant body that references other constants', () => {
    ...
    // label should reference this._const_prefix and this._count()
    expect(output).toContain('this._const_label = () => this._const_prefix + this._count()');
  });
});
