...
describe('transformExpr', () => {
  ...
  it('transforms both signals and computeds in the same expression', () => {
    ...
    expect(result).toBe('this._state.count + this._c_doubled()');
  });
...
describe('transformMethodBody', () => {
  ...
  it('transforms both signal writes and reads in the same body', () => {
    ...
    expect(result).toBe('this._state.count = this._state.count + this._c_doubled()');
  });
...
describe('generateComponent — integration', () => {
  it('generates a complete component with all sections', () => {
    ...
    // 1. Reactive runtime (tree-shaken: __effect and __computed for this component, no __signal)
    ...
    // User effect
    expect(output).toContain("console.log('count changed:', this._state.count)");
  ...
  it('generated output contains reactive runtime, class, connectedCallback, and customElements.define', () => {
    fc.assert(
      fc.property(arbParseResult, (ir) => {
        ...
        // Reactive runtime: no __signal (Proxy replaces it); __computed/__effect only if needed
        expect(output).not.toContain('function __signal(initial)');
        if (ir.computeds.length > 0) expect(output).toContain('function __computed(fn)');
        const needsEffect = ir.effects.length > 0 || ir.ifBlocks?.length > 0 || ir.forBlocks?.length > 0 || ir.modelBindings?.length > 0;
...
describe('Property 11: Codegen Signal/Computed Initialization', () => {
  it('constructor contains Proxy state for each signal and __computed( for each computed', () => {
    fc.assert(
      fc.property(arbParseResult, (ir) => {
        const output = generateComponent(ir);
