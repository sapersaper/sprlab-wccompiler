...
<template>
  ...
  it('should support React/Angular pattern: slot-template-name="..."', async () => {
    const dir = createTempDir();
    try {
      const childContent = `<script>
import { defineComponent } from 'wcc'
export default defineComponent({ tag: 'react-child' })
</script>
<template>
  <div>
    <slot name="header">Default Header</slot>
      ...
      const parentContent = `<script>
import { defineComponent } from 'wcc'
export default defineComponent({ tag: 'react-parent' })
</script>
<template>
  <react-child slot-template-header="<h4>Header via attribute</h4>">
    <p>Default content</p>
  </react-child>
</template>`;

      const childPath = join(dir, 'react-child.wcc');
      const parentPath = join(dir, 'react-parent.wcc');
      writeFileSync(childPath, childContent);
      writeFileSync(parentPath, parentContent);
