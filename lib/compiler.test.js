...
<div>{{count()}}</div><button @click="increment">+</button>
      ...
      // Contains reactive state (Proxy-based, no __signal needed for simple components)
      expect(output).toContain('_state = new Proxy(');
      ...
      // Contains signal initialization in Proxy initial object
...
.container { color: blue; }
      ...
      // 1. Reactive runtime removed in Phase 4
      expect(output).not.toContain('__currentEffect');
