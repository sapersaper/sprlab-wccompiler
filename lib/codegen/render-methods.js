  ...
  for (const ifBlock of ifBlocks) {
    ...
    for (let i = 0; i < ifBlock.branches.length; i++) {
      ...
      for (const innerFor of (branch.forBlocks || [])) {
        ...
        if (innerKeyExpr) {
          // Keyed reconciliation (Vue-style: reuse matching keys)
          const hasNodeEvents = (innerFor.events || []).length > 0;
          ...
          if (hasNodeEvents) {
            ...
            lines.push(`          let itemNode;`);
            lines.push(`          if (__oldMap.has(__key)) {`);
            lines.push(`            itemNode = __oldMap.get(__key);`);
            lines.push(`            __oldMap.delete(__key);`);
            lines.push(`          } else {`);
            lines.push(`            const clone = ${innerVn}_tpl.content.cloneNode(true);`);
            lines.push(`            itemNode = clone.firstChild;`);
            lines.push(`          }`);
          }
          ...
          if (hasNodeEvents) {
            ...
            lines.push(`        for (const n of __newNodes) { if (!n.isConnected) ${innerVn}_anchor.parentNode.insertBefore(n, ${innerVn}_anchor); customElements.upgrade(n); }`);
          }
