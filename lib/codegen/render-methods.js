...
export function generateRenderMethods(lines, parseResult, state = {}) {
  ...
  // ── Collect all forBlocks: top-level + nested inside if-blocks ──
  ...
  // Assign indices and ensure globally unique varNames
  ...
  for (let idx = 0; idx < allForBlocks.length; idx++) {
    const forBlock = allForBlocks[idx];
    ...
    if (keyExpr) {
      ...
      if (hasNodeEvents) {
        ...
        lines.push(`    for (const n of __newNodes) { if (!n.isConnected) this.${vn}_anchor.parentNode.insertBefore(n, this.${vn}_anchor); customElements.upgrade(n); }`);
      }
  ...
  for (const ifBlock of ifBlocks) {
    ...
    for (let i = 0; i < ifBlock.branches.length; i++) {
      ...
      // Model bindings: initial value assignment (updates via __invalidate)
      ...
      for (const innerFor of (branch.forBlocks || [])) {
        ...
        lines.push(`      this.${innerVn}_tpl = document.createElement('template');`);
        lines.push(`      this.${innerVn}_tpl.innerHTML = \`${innerFor.templateHtml}\`;`);
        // Find inner anchor
        lines.push(`      this.${innerVn}_anchor = findAnchor(node, '${innerFor.anchorType}', ${innerFor.anchorIndex});`);
        // Initialize state
        lines.push(`      this.${innerVn}_nodes = [];`);
        lines.push(`      this.${innerVn}_items = [];`);
        // Call the render method for initial render
        lines.push(`      this.__renderEach_${innerFor._renderIndex}();`);
      }
