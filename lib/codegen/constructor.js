...
export function generateConstructor(lines, parseResult, options = {}) {
  ...
  for (const s of signals) {
    // Resolve props.* references to their default values
    let val = s.value;
    if (propsObjectName) {
      const propsRe = new RegExp(`\\b${propsObjectName}\\.(\\w+)`, 'g');
      val = val.replace(propsRe, (match, propName) => {
        const def = propDefs.find(p => p.name === propName);
        return def ? def.default : 'undefined';
      });
    }
    stateEntries.push(`${s.name}: ${val}`);
  }
  for (const md of modelDefs) {
