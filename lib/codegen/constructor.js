...
export function generateConstructor(lines, parseResult, options = {}) {
  ...
  // Check if __effect is needed (nested forBlocks in if-block branches)
  ...
  // Exception: when nested forBlocks in if-blocks use __effect, need get+trap for tracking
  if (stateEntries.length > 0) {
    ...
    lines.push('      {');
    if (needsEffect) {
      lines.push('        get(target, key, receiver) {');
      lines.push('          if (__currentEffect) {');
      lines.push('            if (!target.__subscriptions) target.__subscriptions = {};');
      lines.push('            if (!target.__subscriptions[key]) target.__subscriptions[key] = new Set();');
      lines.push('            target.__subscriptions[key].add(__currentEffect);');
      lines.push('          }');
    ...
    if (needsEffect) {
      lines.push('          if (target.__subscriptions && target.__subscriptions[key]) {');
      lines.push('            for (const eff of target.__subscriptions[key]) {');
      lines.push('              eff();');
      lines.push('            }');
      lines.push('          }');
    }
