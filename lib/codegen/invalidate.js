...
import { generateUpdateOp } from './update-op.js';
...
export function generateInvalidate(lines, depGraph, parseResult, state = {}) {
  ...
  if (depGraph && depGraph.size > 0) {
    /**
     * Execution order within each __invalidate case.
     * 0 = computed (run first — signals/computeds must be recalculated before DOM reads them)
     * 1 = renderIf, renderEach (structural DOM changes — create/destroy branches and items)
     * 2 = renderDynamic, text, show, attr, bool, class, style, model*, childProp (DOM bindings)
     * 3 = watcher (run last — after all DOM is updated, fire user callbacks)
     */
    const typeOrder = { computed: 0, renderIf: 1, renderEach: 1, renderDynamic: 2, text: 2, show: 2, attr: 2, bool: 2, class: 2, style: 2, watcher: 3, modelValue: 2, modelCheckbox: 2, modelRadio: 2, modelProp: 2, childProp: 2 };

    lines.push('  __invalidate(key) {');
    // Only computeds and watchers can fire before connected (pre-connection attribute changes).
    // DOM ops (text, show, attr, renderIf, renderEach, etc.) need guard since DOM refs don't exist yet.
    ...
    for (const [signalKey, entries] of depGraph) {
      ...
      const hasDomOps = sorted.some(e => e.type !== 'computed' && e.type !== 'watcher');
      const hasNonDomOps = sorted.some(e => e.type === 'computed' || e.type === 'watcher');
      lines.push(`      case '${signalKey}':`);
      if (hasDomOps && hasNonDomOps) {
        // Mixed: guard DOM ops, always run non-DOM
        lines.push('        // Non-DOM ops (computeds, watchers) always run');
        for (const entry of sorted) {
          if (entry.type === 'computed' || entry.type === 'watcher') {
