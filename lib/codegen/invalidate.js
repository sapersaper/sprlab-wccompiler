...
export function generateInvalidate(lines, depGraph, parseResult, state = {}) {
  ...
  if (depGraph && depGraph.size > 0) {
    ...
    for (const [signalKey, entries] of depGraph) {
      ...
      const hasDomOps = sorted.some(e => e.type !== 'computed' && e.type !== 'renderIf' && e.type !== 'renderEach' && e.type !== 'watcher');
      const hasNonDomOps = sorted.some(e => e.type === 'computed' || e.type === 'renderIf' || e.type === 'renderEach' || e.type === 'watcher');
      ...
      if (hasDomOps && hasNonDomOps) {
        ...
        for (const entry of sorted) {
          if (entry.type === 'computed' || entry.type === 'renderIf' || entry.type === 'renderEach' || entry.type === 'watcher') {
