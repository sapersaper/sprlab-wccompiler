...
export function generateConnectedCallback(lines, parseResult, opts = {}) {
  ...
  // SSR hydration: adopt existing DOM instead of cloning template
  if (opts.ssr) {
    lines.push('    if (this.children.length > 0) {');
    lines.push('      this.__ssr = true;');
    // For SSR, document.body may not exist in test environments, delay findAnchor
    // to the first __invalidate call. We still need to assign DOM refs.
    // Fall through to standard DOM ref assignments with `this` as root.
    lines.push('    }');
  }
  ...
  const hasForBlockSlots = forBlocks.some(fb => (fb.slots || []).length > 0);
  if (slots.length > 0 || hasForBlockSlots) {
  ...
  // Clone template (skip when SSR content already in DOM)
  lines.push(`    const __root = this.__ssr ? this : __t_${className}.content.cloneNode(true);`);
  ...
  // (skip anchor-dependent setup in SSR mode — anchors don't exist in SSR output)
  if (ifBlocks.length > 0 || forBlocks.length > 0 || dynamicComponents.length > 0) {
    lines.push('    if (!this.__ssr) {');
  }
  ...
  // Append DOM (skip if SSR content already in DOM)
  lines.push("    if (!this.__ssr) { this.innerHTML = ''; }");
  lines.push('    if (!this.__ssr) { this.appendChild(__root); }');

  // Static slot injection (after DOM is appended)
  // Store all scoped slot templates (top-level + inside each loops)
  for (const s of slots) {
    if (s.name && s.slotProps.length > 0) {
      ...
      // Default slot: defer to microtask so children's connectedCallbacks run first.
      // Then MOVE original nodes (not clone) to preserve framework property bindings.
      lines.push(`    if (__defaultSlotNodes.length) {`);
      lines.push(`      const __dsn = __defaultSlotNodes;`);
      lines.push(`      queueMicrotask(() => {`);
      lines.push(`        this.${s.varName}.textContent = '';`);
      lines.push(`        __dsn.forEach(n => this.${s.varName}.appendChild(n));`);
      lines.push(`      });`);
      lines.push(`    }`);
    }
  }

  // Store any additional scoped slot templates from __slotMap
  // (covers scoped slots inside each loops, which are not in parseResult.slots)
  if (hasForBlockSlots) {
    lines.push('    for (const [__sn, __sc] of Object.entries(__slotMap)) {');
    lines.push("      if (__sc.propsExpr && !this['__slotTpl_' + __sn]) {");
    lines.push("        this['__slotTpl_' + __sn] = __sc.content;");
    lines.push('      }');
    lines.push('    }');
  }
  ...
  if (slots.length > 0) {
    ...
    for (const s of slots) {
      if (s.name && s.slotProps.length > 0) {
        ...
        lines.push(`          if (__dn.length) { this.${s.varName}.textContent = ''; __dn.forEach(n => this.${s.varName}.appendChild(n)); }`);
      }
    }
    lines.push('        }');
    lines.push('      });');
