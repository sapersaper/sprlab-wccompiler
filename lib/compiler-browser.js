...
function buildIfBlock(chain, parent, parentPath, idx, signalNames, computedNames, propNames) {
  ...
  return { varName: `__if${idx}`, anchorType: 'if', anchorIndex: idx, _anchorNode: comment, branches };
}
  ...
  function find(node, currentPath) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (el.hasAttribute('each')) {
        ...
        forBlocks.push({
          varName: `__for${currentForIdx}`, itemVar, indexVar, source, keyExpr,
          templateHtml: processedHtml,
          anchorType: 'each', anchorIndex: currentForIdx,
