...
<div>{{ value() > 10 ? 'High' : 'Low' }}</div>
    ...
    expect(code).toMatch(/this\._state\.value\s*>\s*10\s*\?\s*'High'\s*:\s*'Low'/);
  });
...
<div>Hello {{ name() }}</div>
    ...
    expect(code).toMatch(/this\._state\.name\s*\?\?\s*''/);
  });
...
<div>{{ a() + b() }}</div>
    ...
    expect(code).toMatch(/this\._state\.a\s*\+\s*this\._state\.b/);
...
<div>{{ flag1() && flag2() ? 'Both' : 'Not both' }}</div>
    ...
    // Should contain the expression (transformed to this._state prefix)
    expect(code).toMatch(/this\._state\.flag1\s*&&\s*this\._state\.flag2\s*\?\s*'Both'\s*:\s*'Not both'/);
  });
...
<div>{{ score() >= 90 ? 'A' : score() >= 80 ? 'B' : 'C' }}</div>
    ...
    expect(code).toMatch(/this\._state\.score/);
  });
});
