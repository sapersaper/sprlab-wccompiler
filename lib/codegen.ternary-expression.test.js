...
<div>{{ active() ? 'Active' : 'Inactive' }}</div>
    ...
    expect(code).toMatch(/\(this\._state\.active \? 'Active' : 'Inactive'\) \?\? ''/);
...
<div>{{ status() === 'a' ? 'A' : status() === 'b' ? 'B' : 'C' }}</div>
    ...
    expect(code).toContain('this._state.status');
  });
...
<div :class="isActive() ? 'active' : 'inactive'">Content</div>
    ...
    expect(code).toContain('this._state.isActive');
  });

  it('should wrap logical OR expressions before ?? operator', async () => {
    const sfcContent = `<script>
import { defineComponent, signal } from 'wcc'

export default defineComponent({
  tag: 'test-or-operator',
})
...
<div>{{ count() || 'No items' }}</div>
    ...
    console.log('\n=== Generated Code for OR Operator ===');
    const textContentStart = code.indexOf('textContent');
    if (textContentStart !== -1) {
      const snippet = code.substring(textContentStart, textContentStart + 300);
      console.log(snippet);
    ...
    // Logical OR must be wrapped in parentheses before ??
    const match = code.match(/textContent = (.+?) \?\? '';/);
    expect(match).not.toBeNull();
    
    const expr = match[1];
    // Should be wrapped: (this._state.count || 'No items')
    expect(expr.startsWith('(')).toBe(true);
    expect(expr.endsWith(')')).toBe(true);
    expect(expr).toContain('||');
    
    // Verify the exact pattern
    expect(code).toMatch(/\(this\._state\.count \|\| 'No items'\) \?\? ''/);
  });

  it('should wrap logical AND expressions before ?? operator', async () => {
    const sfcContent = `<script>
import { defineComponent, signal } from 'wcc'

export default defineComponent({
  tag: 'test-and-operator',
})
...
<div>{{ flag() && value() }}</div>
    ...
    console.log('\n=== Generated Code for AND Operator ===');
    const textContentStart = code.indexOf('textContent');
    if (textContentStart !== -1) {
      const snippet = code.substring(textContentStart, textContentStart + 300);
      console.log(snippet);
    ...
    // Logical AND must be wrapped in parentheses before ??
    const match = code.match(/textContent = (.+?) \?\? '';/);
    expect(match).not.toBeNull();
  ...
  it('should wrap nullish coalescing expressions before ?? operator', async () => {
    const sfcContent = `<script>
import { defineComponent, signal } from 'wcc'

export default defineComponent({
  tag: 'test-nullish-coalescing',
})
...
<template>
<div>{{ value() ?? fallback() }}</div>
</template>`;

    writeFileSync(join(dir, 'component.wcc'), sfcContent);
    const { code } = await compile(join(dir, 'component.wcc'));

    console.log('\n=== Generated Code for Nullish Coalescing ===');
    const textContentStart = code.indexOf('textContent');
    if (textContentStart !== -1) {
      const snippet = code.substring(textContentStart, textContentStart + 300);
      console.log(snippet);
  ...
  it('should wrap complex expressions with multiple operators', async () => {
    const sfcContent = `<script>
import { defineComponent, signal } from 'wcc'

export default defineComponent({
  tag: 'test-complex-expr',
})

const items = signal([])
const showEmpty = signal(true)
</script>

<template>
<div>{{ items().length > 0 ? items()[0].name : showEmpty() && 'Empty' }}</div>
</template>`;
    ...
    // Complex expressions must be wrapped
    const match = code.match(/textContent = (.+?) \?\? '';/);
    expect(match).not.toBeNull();
    ...
    expect(expr.startsWith('(')).toBe(true);
    expect(expr.endsWith(')')).toBe(true);
    // Should contain both ternary and &&
    expect(expr).toContain('?');
    expect(expr).toContain('&&');
  });
});
