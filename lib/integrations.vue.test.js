...
describe('Vue Integration - wccVuePlugin', () => {
  ...
  describe('Pre-transform: v-model rewriting', () => {
    ...
    it('handles multiple v-model:propName on the same element', () => {
      ...
      // Both handlers merged into a single @wcc:model (no duplicate attributes)
      expect(result).toContain("$event.detail.prop === 'count' && (countRef = $event.detail.value)")
      expect(result).toContain("$event.detail.prop === 'title' && (titleRef = $event.detail.value)")
      // Only ONE @wcc:model attribute (merged with semicolons)
      const matches = result.match(/@wcc:model="/g)
    ...
    it('handles v-model:prop.number alongside other v-model on same element', () => {
      ...
      expect(result).toContain(':title="titleRef"')
      expect(result).toContain('.trim()')
      // Only ONE @wcc:model attribute (merged)
      const matches = result.match(/@wcc:model="/g)
      expect(matches).toHaveLength(1)
    })

    it('handles v-model + v-model:count + v-model:title on same element (QA regression)', () => {
      const input = '<template><wcc-model-fw-test v-model="vueModelValue" v-model:count="vueModelCount" v-model:title="vueModelTitle"></wcc-model-fw-test></template>'
      const result = transform(input, 'test.vue')

      expect(result).toContain(':model-value="vueModelValue"')
      expect(result).toContain(':count="vueModelCount"')
      expect(result).toContain(':title="vueModelTitle"')
      // All three handlers in a single @wcc:model
      expect(result).toContain("$event.detail.prop === 'modelValue'")
      expect(result).toContain("$event.detail.prop === 'count'")
      expect(result).toContain("$event.detail.prop === 'title'")
      // Only ONE @wcc:model attribute
      const matches = result.match(/@wcc:model="/g)
      expect(matches).toHaveLength(1)
      // No duplicate attribute error — no v-model left
      expect(result).not.toContain('v-model')
    })
  })
