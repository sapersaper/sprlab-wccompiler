...
<div each="item in items()" key={{ item.id }}>
  <button @click={{ () => handleClick(item.id) }}>Click</button>
    ...
    // Should have null check pattern with unique variable names
    expect(code).toMatch(/const __evt_target_\d+__/);
    expect(code).toMatch(/if \(__evt_target_\d+__\)/);
    
    // Should NOT have direct addEventListener without null check
    const lines = code.split('\n');
    const handlerLines = lines.filter(line => 
      line.includes('addEventListener') && 
      !line.match(/if \(__evt_target_\d+__\)/) &&
...
<div each="category in categories()" key={{ category.id }}>
  <div each="item in category.items" key={{ item.id }}>
    ...
    // Should have null check pattern for nested loop handlers with unique names
    expect(code).toMatch(/const __evt_target_\d+__/);
    expect(code).toMatch(/if \(__evt_target_\d+__\)/);
  });
...
<div each="item in items()" key={{ item.id }}>
  ...
  <button @click={{ action3 }}>C</button>
    ...
    const nullCheckCount = (code.match(/const __evt_target_\d+__/g) || []).length;
...
<div>
  ...
  <div each="item in items()" key={{ item }}>
    ...
    // Should have null checks with unique variable names
    expect(code).toMatch(/if \(__evt_target_\d+__\)/);
  });
});
