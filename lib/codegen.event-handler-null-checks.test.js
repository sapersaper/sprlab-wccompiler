...
<div each="item in items()" key={{ item.id }}>
  <button @click={{ () => handleClick(item.id) }}>Click</button>
    ...
    // Should have inline null checks (no temporary variables)
    expect(code).toMatch(/if \(node\.childNodes\[\d+\]\) node\.childNodes\[\d+\]\.addEventListener/);
    ...
    const handlerLines = lines.filter(line => 
      ...
      !line.match(/if \(node\.childNodes/) &&
      !line.includes('if (this.__evt_')
    );
    
    // All handler lines should have inline null checks
...
<div each="category in categories()" key={{ category.id }}>
  <div each="item in category.items" key={{ item.id }}>
    ...
    // Should have inline null checks for nested loop handlers (no temporary variables)
    expect(code).toMatch(/if \(innerNode\.childNodes\[\d+\]\) innerNode\.childNodes\[\d+\]\.addEventListener/);
...
<div each="item in items()" key={{ item.id }}>
  <input model="value" />
    ...
    // Should have inline null check for model event listener (same pattern as regular events)
    expect(code).toMatch(/if \(node\.childNodes\[\d+\]\) node\.childNodes\[\d+\]\.addEventListener/);
  });
...
<div each="item in items()" key={{ item.id }}>
  ...
  <button @click={{ action3 }}>C</button>
    ...
    // Count inline null check patterns - should have 3 (one per handler)
    const nullCheckCount = (code.match(/if \(node\.childNodes\[\d+\]\) node\.childNodes\[\d+\]\.addEventListener/g) || []).length;
