...
`;
  ...
  it('wcc build discovers .wcc files alongside .js files', () => {
    const dir = createTempDir();
    const srcDir = join(dir, 'src');
    const distDir = join(dir, 'dist');
    mkdirSync(srcDir, { recursive: true });

    // Create a .js multi-file component
    writeFileSync(
      join(srcDir, 'wcc-legacy.js'),
      `import { defineComponent, signal } from 'wcc'\n\nexport default defineComponent({\n  tag: 'wcc-legacy',\n  template: './wcc-legacy.html',\n  styles: './wcc-legacy.css',\n})\n\nconst val = signal(1)\n`
    );
    writeFileSync(join(srcDir, 'wcc-legacy.html'), '<div>{{val}}</div>');
    writeFileSync(join(srcDir, 'wcc-legacy.css'), 'div { color: red; }');

    // Create a .wcc SFC component
    const sfcSource = `<script>
import { defineComponent, signal } from 'wcc'

export default defineComponent({ tag: 'wcc-sfc' })
...
`;
    writeFileSync(join(srcDir, 'wcc-sfc.wcc'), sfcSource);
    ...
    expect(existsSync(join(distDir, 'wcc-legacy.js'))).toBe(true);
    expect(existsSync(join(distDir, 'wcc-sfc.js'))).toBe(true);
  });
});
