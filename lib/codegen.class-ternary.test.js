...
<div :class="theme() === 'light' ? 'light-theme' : 'dark-theme'">Test</div>
    ...
    expect(code).toContain('this._state.theme');
    
    // The ternary should be properly formed (with or without parentheses)
    expect(code).toMatch(/className\s*=\s*\(?\s*this\._state\.theme\s*===\s*'light'\s*\?\s*'light-theme'\s*:\s*'dark-theme'/);
  });
...
<div :class="isActive() ? (size() === 'large' ? 'active-large' : 'active-small') : 'inactive'">Test</div>
    ...
    expect(code).toContain('this._state.size');
  });
...
<div :class='mode() === "test" ? "test-mode-active" : "test-mode-inactive"'>Test</div>
    ...
    // Should transform signal calls
    expect(code).toContain('this._state.mode');
  });
...
<div :class="count() > 0 ? hasItems() : noItems()">Test</div>
    ...
    expect(code).toContain('this._state.hasItems');
    expect(code).toContain('this._state.noItems');
  });

  it('should handle template literals in :class binding', async () => {
    const sfcContent = `<script>
import { defineComponent, signal } from 'wcc'

export default defineComponent({
  tag: 'test-template-literal',
})
...
<template>
<div :class="\`\${theme()}-theme \${size()}-size\`">Test</div>
</template>`;

    writeFileSync(join(dir, 'component.wcc'), sfcContent);
    const { code } = await compile(join(dir, 'component.wcc'));

    console.log('\n=== Generated Code for Template Literal ===');
    const classNameStart = code.indexOf('.className =');
    if (classNameStart !== -1) {
      const snippet = code.substring(classNameStart, classNameStart + 250);
      console.log(snippet);
    }

    // Should transform method calls inside template literal expressions
    expect(code).toContain('this._state.theme');
    expect(code).toContain('this._state.size');
    
    // Should preserve the template literal structure
    expect(code).toMatch(/`\$\{this\._state\.theme\}-theme/);
    expect(code).toMatch(/\$\{this\._state\.size\}-size`/);
    
    // Should NOT have bare function calls without this._ prefix
    expect(code).not.toMatch(/[^_]theme\(\)/);
    expect(code).not.toMatch(/[^_]size\(\)/);
  });
});
