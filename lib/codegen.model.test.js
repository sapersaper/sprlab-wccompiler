...
describe('Feature: model-directive, Property 4: Codegen Signal-to-DOM Effect Structure', () => {
  it('generates DOM refs in constructor and __effect per ModelBinding in connectedCallback', () => {
    fc.assert(
      fc.property(arbParseResultWithModel, (ir) => {
        const output = generateComponent(ir);

        // Constructor: DOM element reference per ModelBinding
        ...
        // DOM refs must appear before appendChild
        const constructorSection = output.slice(0, output.indexOf('connectedCallback()'));
        const appendIdx = constructorSection.indexOf('this.appendChild(__root)');
        for (const mb of ir.modelBindings) {
          const refIdx = constructorSection.indexOf(`this.${mb.varName} = `);
