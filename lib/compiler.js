...
import { walkTree, processIfChains, processForBlocks, processDynamicComponents, detectRefs } from './tree-walker.js';
import { generateComponent } from './codegen.js';
import { generateSSR } from './codegen/ssr.js';
...
import { buildParseResult } from './parser/sfc-descriptor.js';
...
async function compileSFC(filePath, config) {
  ...
  // Self-referencing components: treat own tag as leaf (don't walk into children)
  const knownTags = new Set([tagName]);

  const forBlocks = processForBlocks(rootEl, [], signalNames, computedNames, propNamesSet, knownTags);
  const ifBlocks = processIfChains(rootEl, [], signalNames, computedNames, propNamesSet, knownTags);
  const dynamicComponents = processDynamicComponents(rootEl, [], signalNames, computedNames, propNamesSet, knownTags);

  rootEl.normalize();

  const { bindings, events, showBindings, modelBindings, modelPropBindings, attrBindings, slots, childComponents } = walkTree(rootEl, signalNames, computedNames, propNamesSet, new Set(constantVars.map(v => v.name)), knownTags);
  ...
  // 20. Generate component (zero-runtime: always inline, no shared runtime)
  const genOptions = { ...config, sourceFile: fileName, runtimeImportPath: undefined };
  const code = generateComponent(parseResult, genOptions);
  const ssrCode = config?.ssr ? generateSSR(parseResult) : null;
  return { code, ssrCode };
}
 ...
 * @param {string} filePath — Absolute or relative path to the .wcc file
 * @param {object} [config] — Optional config (reserved for future options)
 * @returns {Promise<{code: string}>} The generated JavaScript component code
 */
