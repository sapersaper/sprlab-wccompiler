...
async function compileSFC(filePath, config) {
  ...
  const dynamicComponents = processDynamicComponents(rootEl, [], signalNames, computedNames, propNamesSet);

  rootEl.normalize();

  const { bindings, events, showBindings, modelBindings, modelPropBindings, attrBindings, slots, childComponents } = walkTree(rootEl, signalNames, computedNames, propNamesSet, new Set(constantVars.map(v => v.name)));
