...
export function transformMethodBody(body, signalNames, computedNames, propsObjectName = null, propNames = new Set(), emitsObjectName = null, refVarNames = [], constantNames = [], modelVarMap = new Map(), methodNames = []) {
  ...
  // Now safe because reads are already transformed (no nested parens in value)
  for (const name of signalNames) {
    if (propsObjectName && name === propsObjectName) continue;
    if (emitsObjectName && name === emitsObjectName) continue;
...
export function transformForExpr(expr, itemVar, indexVar, propsSet, rootVarNames, computedNames, methodNames = [], constantNames = []) {
  ...
  for (const p of propsSet) {
    ...
    r = r.replace(new RegExp(`\\b${p}\\b(?!\\()`, 'g'), `this._state.${p}`);
  }
 ...
 * @param {string[]} [constantNames]
 */
function generateItemSetup(lines, forBlock, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames = []) {
  const indent = '        ';
  ...
  for (const ifBlock of (forBlock.ifBlocks || [])) {
    ...
    // Generate nested forBlocks from each branch INSIDE the conditional block
    // Each branch may have its own nested each loops with anchor paths relative to the branch node
    for (let branchIdx = 0; branchIdx < branches.length; branchIdx++) {
      const branch = branches[branchIdx];
      const branchForBlocks = branch.forBlocks || [];
      if (branchForBlocks.length === 0) continue;

      const branchGuard = branchIdx === 0 ? 'if' : 'else if';
      lines.push(`${indent}    ${branchGuard} (${vn}_branch === ${branchIdx}) {`);

      for (const innerFor of branchForBlocks) {
        ...
        // Find inner anchor INSIDE the conditional branch node
        lines.push(`${indent}      const ${innerVn}_anchor = ${pathExpr(innerFor.anchorPath, `${vn}_node`)};`);

        // Transform the inner source expression
        const innerSourceExpr = transformForExpr(innerSource, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames);
        const innerSourceIsStatic = isStaticForExpr(innerSource, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, constantNames);
    ...
    if (hasSetup) {
      ...
      for (let i = 0; i < branches.length; i++) {
        ...
        for (const mb of (branch.modelBindings || [])) {
          ...
          lines.push(`${indent}      });`);
          if (mb.prop === 'checked' && mb.radioValue === null) {
            lines.push(`${indent}      ${nodeRef}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = e.target.checked; });`);
          } else if (mb.coerce) {
            lines.push(`${indent}      ${nodeRef}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = Number(e.target.value); });`);
          } else {
            lines.push(`${indent}      ${nodeRef}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = e.target.value; });`);
          }
        }

        // Dynamic components inside this branch (within a forBlock's nested ifBlock)
        for (const dyn of (branch.dynamicComponents || [])) {
          const dvn = dyn.varName;
          const anchorRef = pathExpr(dyn.anchorPath, `${vn}_node`);
          const isExprTransformed = isStaticForExpr(dyn.isExpression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet)
            ? dyn.isExpression
            : transformForExpr(dyn.isExpression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames);
          ...
          for (const prop of dyn.props) {
            const propExpr = isStaticForExpr(prop.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet)
              ? prop.expression
              : transformForExpr(prop.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames);
  ...
  // Nested dynamic components (<component :is>) inside each items
  for (const dyn of (forBlock.dynamicComponents || [])) {
    const dvn = dyn.varName;
    const anchorRef = pathExpr(dyn.anchorPath, 'node');

    // Transform the :is expression using the loop context
    const isExprTransformed = isStaticForExpr(dyn.isExpression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet)
      ? dyn.isExpression
      : transformForExpr(dyn.isExpression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames);
...
export function buildDepGraph(parseResult, transformContext) {
  ...
  for (const b of bindings) {
    ...
    // Constant function bindings stay in __effect (they call signals internally)
    if (b.type === 'constant') {
    ...
    else rawExpr = b.name; // expression/method type
...
export function generateComponent(parseResult, options = {}) {
  ...
  // needsEffect is true only when complex features exist or bindings remain in __effect
  const hasEffectBindings = classifiedEffectBindings.text.length > 0 || classifiedEffectBindings.show.length > 0 || classifiedEffectBindings.attr.length > 0;
  const needsEffect = effects.length > 0 || hasEffectBindings || modelBindings.length > 0 || modelPropBindings.length > 0 || ifBlocks.length > 0 || forBlocks.length > 0 || watchers.length > 0 || childComponents.length > 0 || dynamicComponents.length > 0 || slots.some(s => s.slotProps.length > 0);
  ...
  if (options.runtimeImportPath) {
    ...
    // When the Proxy get trap is generated (needsEffect), we need the runtime globals
    if (needsEffect || needsComputed) {
  ...
  // Collect initial state entries from props, signals, and models
  const stateEntries = [];
  ...
  if (stateEntries.length > 0) {
    ...
    lines.push('      {');
    if (needsEffect) {
      // Complex component: needs get trap for effect tracking
      lines.push("        get(target, key) {");
      lines.push("          if (__currentEffect && typeof key === 'string' && key !== '__subs') {");
      lines.push("            if (!target.__subs) target.__subs = {};");
      lines.push("            if (!target.__subs[key]) target.__subs[key] = new Set();");
      lines.push("            target.__subs[key].add(__currentEffect);");
      lines.push("          }");
      lines.push("          return target[key];");
      lines.push("        },");
      lines.push('        set(target, key, value) {');
      lines.push("          if (key === '__subs') { target[key] = value; return true; }");
      lines.push('          if (target[key] === value) return true;');
      lines.push('          target[key] = value;');
      lines.push("          if (target.__subs && target.__subs[key]) {");
      lines.push("            if (__batchDepth > 0) {");
      lines.push("              for (const fn of target.__subs[key]) __pendingEffects.add(fn);");
      lines.push("            } else {");
      lines.push("              for (const fn of [...target.__subs[key]]) fn();");
      lines.push("            }");
      lines.push("          }");
      ...
      // Simple component: only set trap needed
      lines.push('        set(target, key, value) {');
      lines.push('          if (target[key] === value) return true;');
      lines.push('          target[key] = value;');
      lines.push("          self.__invalidate(key);");
  ...
  // Only create disposers array if we have complex features that need __effect
  const hasComplexFeatures = needsEffect;
  if (hasComplexFeatures) {
    lines.push('    this.__disposers = [];');
  }
  lines.push('');

  // Binding effects — only for bindings that remain in __effect (computed-dependent, method-referencing)
  // Simple bindings are handled by __invalidate
  if (classifiedEffectBindings.text.length > 0) comment('Text bindings (effect)');
  for (const b of classifiedEffectBindings.text) {
    if (b.type === 'prop') {
      lines.push('    this.__disposers.push(__effect(() => {');
      lines.push(`      this.${b.varName}.textContent = this._state.${b.name} ?? '';`);
      lines.push('    }));');
    } else if (b.type === 'signal') {
      const modelPropName = modelVarMap.get(b.name);
      const signalRef = modelPropName ? `this._state.${modelPropName}` : `this._state.${b.name}`;
      lines.push('    this.__disposers.push(__effect(() => {');
      ...
      // Function constants reference signals and need reactive effects
      const constDef = constantVars.find(c => c.name === b.name);
      ...
      let ref;
      const simplePropMatch = propsObjectName && b.name.match(new RegExp(`^${propsObjectName}\\.(\\w+)$`));
      if (simplePropMatch && propNames.has(simplePropMatch[1])) {
        ref = `this._state.${simplePropMatch[1]}`;
      } else {
        const isComplexExpr = /[?:&|<>=!+\-*/%]/.test(b.name) || b.name.includes('(');
        const exprToTransform = isComplexExpr ? b.name : `${b.name}()`;
        ref = transformExpr(exprToTransform, signalNames, computedNames, propsObjectName, propNames, emitsObjectName, constantNames, methodNames, modelVarMap);
      }
      lines.push('    this.__disposers.push(__effect(() => {');
  ...
  // Static constant bindings (set once, no effect needed) — from bindings not in depGraph or effectBindings
  for (const b of bindings) {
    if (b.type === 'constant') {
      const constDef = constantVars.find(c => c.name === b.name);
      const isFunction = constDef && /^\s*(\(|function\b)/.test(constDef.value);
      if (!isFunction) {
        // Check if it's not already in effectBindings
        const inEffect = classifiedEffectBindings.text.some(eb => eb.varName === b.varName);
        if (!inEffect) {
          lines.push(`    this.${b.varName}.textContent = this._const_${b.name} ?? '';`);
        }
      }
  ...
  for (const mb of modelBindings) {
    if (mb.prop === 'checked' && mb.radioValue !== null) {
      ...
      lines.push('    this.__disposers.push(__effect(() => {');
      lines.push(`      this.${mb.varName}.checked = (this._state.${mb.signal} === '${mb.radioValue}');`);
      lines.push('    }));');
    } else if (mb.prop === 'checked') {
      // Checkbox: coerce to boolean
      lines.push('    this.__disposers.push(__effect(() => {');
      lines.push(`      this.${mb.varName}.checked = !!this._state.${mb.signal};`);
      lines.push('    }));');
    } else {
      // Value-based (text, number, textarea, select): nullish coalesce to ''
      lines.push('    this.__disposers.push(__effect(() => {');
      lines.push(`      this.${mb.varName}.value = this._state.${mb.signal} ?? '';`);
      lines.push('    }));');
    }
  }
  ...
  for (const mb of modelBindings) {
    if (mb.prop === 'checked' && mb.radioValue === null) {
      ...
      lines.push(`    if (this.${mb.varName}) this.${mb.varName}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = e.target.checked; }, { signal: this.__ac.signal });`);
    } else if (mb.coerce) {
      // Number input: wrap in Number()
      lines.push(`    if (this.${mb.varName}) this.${mb.varName}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = Number(e.target.value); }, { signal: this.__ac.signal });`);
    } else {
      // All others: read e.target.value
      lines.push(`    if (this.${mb.varName}) this.${mb.varName}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = e.target.value; }, { signal: this.__ac.signal });`);
    }
  }
  ...
  for (const mpb of modelPropBindings) {
    ...
    // If the signal is a model var, use this._state.propName; otherwise use this._state.signalName
    const isModelVar = modelVarMap.has(mpb.signal);
    const readExpr = isModelVar
      ? `this._state.${modelVarMap.get(mpb.signal)}`
      : `this._state.${mpb.signal}`;
    const writeExpr = (val) => isModelVar
      ? `this._state.${modelVarMap.get(mpb.signal)} = ${val}`
      : `this._state.${mpb.signal} = ${val}`;
  ...
  // Attr binding effects — only for attr bindings that remain in __effect
  for (const ab of classifiedEffectBindings.attr) {
    ...
    if (ab.kind === 'attr') {
      ...
      if (ab.expression.trimStart().startsWith('{')) {
        ...
        const staticPrefix = ab.staticValue ? `'${ab.staticValue} ' + ` : '';
        lines.push('    this.__disposers.push(__effect(() => {');
        lines.push(`      this.${ab.varName}.className = ${staticPrefix}(${expr}).join(' ');`);
        lines.push('    }));');
      } else {
        // String expression: set className, preserve static classes
        ...
        const staticPrefix = ab.staticValue ? `'${ab.staticValue} ' + ` : '';
        lines.push('    this.__disposers.push(__effect(() => {');
        lines.push(`      this.${ab.varName}.className = ${staticPrefix}${wrapTernaryExpr(expr)};`);
        lines.push('    }));');
      }
    } else if (ab.kind === 'style') {
      if (ab.expression.trimStart().startsWith('{')) {
        ...
        const staticPrefix = ab.staticValue ? `'${ab.staticValue}; ' + ` : '';
        lines.push('    this.__disposers.push(__effect(() => {');
        lines.push(`      this.${ab.varName}.style.cssText = ${staticPrefix}${expr};`);
        lines.push('    }));');
      }
    }
  }
  ...
  for (const forBlock of forBlocks) {
    ...
    if (keyExpr) {
      ...
      generateItemSetup(lines, forBlock, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames);
  ...
  // Initial render of all simple bindings via __invalidate
  if (depGraph.size > 0) {
  ...
  if (depGraph.size > 0) {
    ...
    // Wildcard case: all unique update operations (deduplicated)
    lines.push("      case '*':");
  ...
  for (const ifBlock of ifBlocks) {
    ...
    for (let i = 0; i < ifBlock.branches.length; i++) {
      ...
      for (const b of branch.bindings) {
        ...
        if (b.type === 'prop') {
          lines.push(`      __effect(() => { ${b.varName}.textContent = this._state.${b.name} ?? ''; });`);
        } else if (b.type === 'signal') {
          const modelPropName = modelVarMap.get(b.name);
          const signalRef = modelPropName ? `this._state.${modelPropName}` : `this._state.${b.name}`;
          ...
          lines.push(`      ${b.varName}.textContent = this._const_${b.name} ?? '';`);
        } else {
          // method/expression type — check for simple props.x pattern
          let ref;
          const simplePropMatch2 = propsObjectName && b.name.match(new RegExp(`^${propsObjectName}\\.(\\w+)$`));
          if (simplePropMatch2 && propNames.has(simplePropMatch2[1])) {
            ref = `this._state.${simplePropMatch2[1]}`;
          } else {
            // For complex expressions (ternary, logical, etc.), use transformExpr directly
            const isComplexExpr2 = /[?:&|<>=!+\-*/%]/.test(b.name) || b.name.includes('(');
            const exprToTransform2 = isComplexExpr2 ? b.name : `${b.name}()`;
            ref = transformExpr(exprToTransform2, signalNames, computedNames, propsObjectName, propNames, emitsObjectName, constantNames, methodNames, modelVarMap);
          }
      ...
      for (const mb of (branch.modelBindings || [])) {
        ...
        // Listener (DOM → signal)
        if (mb.prop === 'checked' && mb.radioValue === null) {
          lines.push(`      ${mb.varName}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = e.target.checked; });`);
        } else if (mb.coerce) {
          lines.push(`      ${mb.varName}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = Number(e.target.value); });`);
        } else {
          lines.push(`      ${mb.varName}.addEventListener('${mb.event}', (e) => { this._state.${mb.signal} = e.target.value; });`);
        }
      }
