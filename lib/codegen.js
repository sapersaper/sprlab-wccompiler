...
export function transformExpr(expr, signalNames, computedNames, propsObjectName = null, propNames = new Set(), emitsObjectName = null, constantNames = [], methodNames = [], modelVarMap = new Map()) {
  ...
  // Step 1: Match simple string literals (single quotes and double quotes)
  ...
  result = result.replace(/`([^`]*)`/g, (match, content) => {
    ...
    for (let i = 0; i < parts.length; i++) {
      if (parts[i].length > 0) {
        const placeholder = `__TL_${placeholderIdx++}__`;
        templateLiteralParts.set(placeholder, parts[i]);
        rebuilt += placeholder;
      }
      if (exprIndex < exprs.length) rebuilt += exprs[exprIndex++];
    }
    rebuilt += '`';
    return rebuilt;
  });
  ...
  // BUG-0011 FIX: Restore string literals and template literal parts from placeholders
...
export function isStaticForBinding(name, itemVar, indexVar) {
  // BUG-0018 FIX: Expressions with operators are never static - need reactive wrapping
...
export function generateComponent(parseResult, options = {}) {
  ...
  for (const forBlock of forBlocks) {
    ...
    if (keyExpr) {
      ...
      // BUG-0012 FIX: Recreate nodes to get fresh effects (stale references on reuse)
