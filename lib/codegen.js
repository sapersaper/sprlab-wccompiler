...
function generateItemSetup(lines, forBlock, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames = [], modelVarMap = new Map(), indentOverride = null) {
  ...
  // Track dynamic components already generated inside if-blocks to avoid double-generation
  const generatedDynVars = new Set();
  ...
  for (const ifBlock of (forBlock.ifBlocks || [])) {
    ...
    for (let i = 0; i < ifBlock.branches.length; i++) {
      ...
      for (const innerFor2 of (branch.forBlocks || [])) {
        ...
        const innerAnchorRef2 = `__parent_anchor_${innerVn2}`;
        ...
        lines.push(`${indent}      const node = clone2.content.firstChild;`);
        generateItemSetup(lines, innerFor2, innerItemVar2, innerIndexVar2, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames, modelVarMap, indent + '    ');
        lines.push(`${indent}      ${innerAnchorRef2}.parentNode.insertBefore(node, ${innerAnchorRef2});`);
        lines.push(`${indent}    });`);
      }
      // Nested dynamic components inside if-branch
      for (const dyn of (branch.dynamicComponents || [])) {
        const dvn = dyn.varName;
        generatedDynVars.add(dvn);
  ...
  for (const innerFor of (forBlock.forBlocks || [])) {
    ...
    const innerAnchorVar = `__outer_anchor_${innerVn}`;
    ...
    lines.push(`${indent}    const node = clone.content.firstChild;`);
    generateItemSetup(lines, innerFor, innerItemVar, innerIndexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames, modelVarMap, indent + '  ');
    lines.push(`${indent}    ${innerAnchorVar}.parentNode.insertBefore(node, ${innerAnchorVar});`);
    lines.push(`${indent}  });`);
  }

  // Dynamic components (<component :is>) inside each item
  for (const dyn of (forBlock.dynamicComponents || [])) {
    // Skip if already generated inside an if-block branch
    if (generatedDynVars.has(dyn.varName)) continue;
    const dvn = dyn.varName;
