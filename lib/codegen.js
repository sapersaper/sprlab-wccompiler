...
function generateItemSetup(lines, forBlock, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames) {
  ...
  for (const sb of forBlock.showBindings) {
    ...
    // Check if this is an inline conditional (if attribute inside loop)
    if (sb.isConditional) {
      // Generate inline conditional logic: wrap element creation/removal in effect
      ...
      lines.push(`${indent}      ${nodeRef}.style.display = '';`);
      lines.push(`${indent}    } else {`);
      lines.push(`${indent}      ${nodeRef}.style.display = 'none';`);
      lines.push(`${indent}    }`);
      lines.push(`${indent}  });`);
    } else {
      // Regular show binding - use CSS display
  ...
  for (const ifBlock of (forBlock.ifBlocks || [])) {
    ...
    // 3.2: Generate per-item conditional evaluation and DOM insertion (REACTIVE - wrapped in __effect)
    lines.push(`${indent}  let ${vn}_active = null;`); // Track active branch
    lines.push(`${indent}  let ${vn}_current = null;`); // Track current DOM node
    lines.push(`${indent}  __effect(() => {`);
    lines.push(`${indent}    let ${vn}_branch = null;`);
    for (let i = 0; i < branches.length; i++) {
      const branch = branches[i];
      if (branch.type === 'if') {
        const expr = transformForExpr(branch.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames);
        lines.push(`${indent}    if (${expr}) { ${vn}_branch = ${i}; }`);
      } else if (branch.type === 'else-if') {
        const expr = transformForExpr(branch.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames);
        lines.push(`${indent}    else if (${expr}) { ${vn}_branch = ${i}; }`);
      } else {
        // else
    ...
    // Check if branch changed - if not, skip DOM updates
    lines.push(`${indent}    if (${vn}_branch === ${vn}_active) return;`);
    
    // Remove previous branch node if exists
    lines.push(`${indent}    if (${vn}_current) { ${vn}_current.remove(); ${vn}_current = null; }`);

    // 3.3: Insert only the matching branch node
    ...
    lines.push(`${indent}    ${vn}_current = ${vn}_node;`); // Track current node
    ...
    lines.push(`${indent}    ${vn}_active = ${vn}_branch;`);
    lines.push(`${indent}  }`);
    lines.push(`${indent}  });`); // Close __effect
  }
