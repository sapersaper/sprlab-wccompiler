...
function generateItemSetup(lines, forBlock, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames) {
  ...
  for (const e of forBlock.events) {
    ...
    lines.push(`${indent}  const __evt_target_${__evt_counter++}__ = ${nodeRef};`);
    lines.push(`${indent}  if (__evt_target_${__evt_counter - 1}__) __evt_target_${__evt_counter - 1}__.addEventListener('${e.event}', ${handlerExpr});`);
  }
  ...
  for (const mb of (forBlock.modelBindings || [])) {
    ...
    lines.push(`${indent}  });`);
    if (mb.prop === 'checked' && mb.radioValue === null) {
      lines.push(`${indent}  const __model_target_${__model_counter++}__ = ${nodeRef};`);
      lines.push(`${indent}  if (__model_target_${__model_counter - 1}__) __model_target_${__model_counter - 1}__.addEventListener('${mb.event}', (e) => { this._${mb.signal}(e.target.checked); });`);
    } else if (mb.coerce) {
      lines.push(`${indent}  const __model_target_${__model_counter++}__ = ${nodeRef};`);
      lines.push(`${indent}  if (__model_target_${__model_counter - 1}__) __model_target_${__model_counter - 1}__.addEventListener('${mb.event}', (e) => { this._${mb.signal}(Number(e.target.value)); });`);
    } else {
      lines.push(`${indent}  const __model_target_${__model_counter++}__ = ${nodeRef};`);
      lines.push(`${indent}  if (__model_target_${__model_counter - 1}__) __model_target_${__model_counter - 1}__.addEventListener('${mb.event}', (e) => { this._${mb.signal}(e.target.value); });`);
    }
  }
