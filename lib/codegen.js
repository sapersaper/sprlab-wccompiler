...
function generateItemSetup(lines, forBlock, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames = []) {
  ...
  for (const ab of forBlock.attrBindings) {
    ...
    if (ab.kind === 'class') {
      ...
      if (ab.expression.trimStart().startsWith('{')) {
        ...
        if (isStaticForExpr(ab.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet)) {
          lines.push(`${indent}  ${nodeRef}.className = (${ab.expression}).join(' ');`);
        } else {
          const expr = transformForExpr(ab.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames);
          lines.push(`${indent}  __effect(() => { ${nodeRef}.className = (${expr}).join(' '); });`);
        }
      } else {
        // String expression: set className
        if (isStaticForExpr(ab.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet)) {
          lines.push(`${indent}  ${nodeRef}.className = ${ab.expression};`);
        } else {
          const expr = transformForExpr(ab.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames);
          lines.push(`${indent}  __effect(() => { ${nodeRef}.className = ${wrapTernaryExpr(expr)}; });`);
        }
      }
      ...
      if (ab.expression.trimStart().startsWith('{')) {
        ...
        if (isStaticForExpr(ab.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet)) {
          lines.push(`${indent}  ${nodeRef}.style.cssText = ${ab.expression};`);
        } else {
          const expr = transformForExpr(ab.expression, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames);
          lines.push(`${indent}  __effect(() => { ${nodeRef}.style.cssText = ${expr}; });`);
        }
      }
    } else {
  ...
  for (const mb of (forBlock.modelBindings || [])) {
    ...
    lines.push(`${indent}  });`);
    if (mb.prop === 'checked' && mb.radioValue === null) {
      lines.push(`${indent}  if (${nodeRef}) ${nodeRef}.addEventListener('${mb.event}', (e) => { this._${mb.signal}(e.target.checked); });`);
    } else if (mb.coerce) {
      lines.push(`${indent}  if (${nodeRef}) ${nodeRef}.addEventListener('${mb.event}', (e) => { this._${mb.signal}(Number(e.target.value)); });`);
    } else {
      lines.push(`${indent}  if (${nodeRef}) ${nodeRef}.addEventListener('${mb.event}', (e) => { this._${mb.signal}(e.target.value); });`);
    }
  }
...
export function generateComponent(parseResult, options = {}) {
  ...
  for (const b of bindings) {
    if (b.type === 'prop') {
      ...
      // Check if this constant is a function (arrow function or function expression)
      ...
      if (isFunction) {
        // Function constant: call it inside an effect (it reads signals internally)
        lines.push('    this.__disposers.push(__effect(() => {');
        lines.push(`      this.${b.varName}.textContent = this._const_${b.name}() ?? '';`);
        lines.push('    }));');
      } else {
        // Value constant: set once, no effect needed
        lines.push(`    this.${b.varName}.textContent = this._const_${b.name} ?? '';`);
      }
    } else {
      // method/expression type — check if it's a simple props.x access or a complex expression
      let ref;
      // Only treat as simple prop access if it's EXACTLY "props.propName" (no operators, no extra chars)
      ...
      if (simplePropMatch && propNames.has(simplePropMatch[1])) {
        ref = `this._s_${simplePropMatch[1]}()`;
      } else {
        ...
        // But for complex expressions (ternary, logical, etc.), don't append ()
      ...
      for (const b of branch.bindings) {
        ...
        if (b.type === 'prop') {
          ...
          if (simplePropMatch2 && propNames.has(simplePropMatch2[1])) {
            ref = `this._s_${simplePropMatch2[1]}()`;
          } else {
