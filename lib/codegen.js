    ...
    for (let i = 0; i < ifBlock.branches.length; i++) {
      ...
      for (const innerFor2 of (branch.forBlocks || [])) {
        ...
        for (const e2 of (innerFor2.events || [])) {
          ...
          lines.push(`${indent}      ${nodeRef2}.addEventListener('${e2.event}', ${handlerExpr2});`);
        }
...
export function generateComponent(parseResult, options = {}) {
  ...
  // Phase 3: each-blocks now handled by __renderEach_N → no longer need __effect
  const needsEffect = effects.length > 0 || hasEffectBindings || modelBindings.length > 0 || modelPropBindings.length > 0 || childComponents.length > 0 || dynamicComponents.length > 0 || slots.some(s => s.slotProps.length > 0);
  ...
  // Phase 3: each-blocks are now handled by __renderEach_N methods called from __invalidate.
  // Template creation, anchor reference, and state initialization are kept in connectedCallback.

  // Phase 3: each-blocks are now handled by __renderEach_N methods called from __invalidate.
  // Template creation, anchor reference, and state initialization (__forN_tpl, __forN_anchor,
  // __forN_nodes, __forN_items) are kept in connectedCallback but the __effect wrapper is removed.
  ...
  if (depGraph.size > 0) {
    ...
    const typeOrder = { computed: 0, renderIf: 1, renderEach: 1, text: 2, show: 2, attr: 2, bool: 2, class: 2, style: 2, watcher: 3 };
    ...
    for (const [, entries] of depGraph) {
      for (const entry of entries) {
        if (entry.type === 'computed' || entry.type === 'renderIf' || entry.type === 'renderEach' || entry.type === 'watcher') continue;
