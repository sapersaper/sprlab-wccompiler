 ...
 * @param {string[]} [constantNames]
 */
function generateItemSetup(lines, forBlock, itemVar, indexVar, propNames, signalNamesSet, computedNamesSet, methodNames, constantNames = [], modelVarMap = new Map()) {
  const indent = '        ';
    ...
    for (let branchIdx = 0; branchIdx < branches.length; branchIdx++) {
      ...
      for (const innerFor of branchForBlocks) {
        ...
        if (innerKeyExpr) {
          ...
          generateNestedItemSetup(lines, innerFor, itemVar, indexVar, innerItemVar, innerIndexVar, propNames, signalNamesSet, computedNamesSet, methodNames, indent + '        ', modelVarMap);
    ...
    if (hasSetup) {
      ...
      for (let i = 0; i < branches.length; i++) {
        ...
        for (const mb of (branch.modelBindings || [])) {
          ...
          lines.push(`${indent}      });`);
          const mbAssign2 = modelVarMap.has(mb.signal)
            ? `this._modelSet_${mb.signal}(`
            : `this._state.${mb.signal} = `;
          if (mb.prop === 'checked' && mb.radioValue === null) {
            lines.push(`${indent}      ${nodeRef}.addEventListener('${mb.event}', (e) => { ${mbAssign2}e.target.checked${modelVarMap.has(mb.signal) ? ')' : ';' } });`);
          } else if (mb.coerce) {
            lines.push(`${indent}      ${nodeRef}.addEventListener('${mb.event}', (e) => { ${mbAssign2}Number(e.target.value)${modelVarMap.has(mb.signal) ? ')' : ';' } });`);
          } else {
            lines.push(`${indent}      ${nodeRef}.addEventListener('${mb.event}', (e) => { ${mbAssign2}e.target.value${modelVarMap.has(mb.signal) ? ')' : ';' } });`);
          }
        }
...
export function buildDepGraph(parseResult, transformContext) {
  ...
  // Extended data from parseResult (if-blocks, computeds, watchers)
  ...
  for (const b of bindings) {
    ...
    if (b.type === 'signal') rawExpr = b.name;
      ...
      // Method bindings always go to __effect (dynamic, can't statically trace)
      effectBindings.text.push(b);
      continue;
    }
...
function generateUpdateOp(entry, lines, indent) {
  switch (entry.type) {
    case 'text':
      if (entry.ifPathExpr) {
        // Internal binding inside an if-block branch — must have existence guard
...
export function generateComponent(parseResult, options = {}) {
  ...
  // Phase 2: if-blocks, computeds, and watchers are now handled by __invalidate
  ...
  const needsEffect = effects.length > 0 || hasEffectBindings || modelBindings.length > 0 || modelPropBindings.length > 0 || forBlocks.length > 0 || childComponents.length > 0 || dynamicComponents.length > 0 || slots.some(s => s.slotProps.length > 0);
  ...
  // Phase 2: if-blocks are now handled by __renderIf_N methods called from __invalidate.
  // Template creation, anchor reference, and state initialization (__ifN_current, __ifN_active)
  // are kept in connectedCallback but the __effect wrapper is removed.
  ...
  if (depGraph.size > 0) {
    ...
    const typeOrder = { computed: 0, renderIf: 1, text: 2, show: 2, attr: 2, bool: 2, class: 2, style: 2, watcher: 3 };
    ...
    for (const [signalKey, entries] of depGraph) {
      ...
      const hasDomOps = sorted.some(e => e.type !== 'computed' && e.type !== 'renderIf' && e.type !== 'watcher');
      const hasNonDomOps = sorted.some(e => e.type === 'computed' || e.type === 'renderIf' || e.type === 'watcher');
      ...
      if (hasDomOps && hasNonDomOps) {
        ...
        for (const entry of sorted) {
          if (entry.type === 'computed' || entry.type === 'renderIf' || entry.type === 'watcher') {
    ...
    for (const [, entries] of depGraph) {
      for (const entry of entries) {
        if (entry.type === 'computed' || entry.type === 'renderIf' || entry.type === 'watcher') continue;
    ...
    for (let idx = 0; idx < watchers.length; idx++) {
      ...
      if (w.kind === 'signal') {
        ...
        addSeenOp({ type: 'watcher', watcherIndex: idx, watcherKind: 'getter', prevName: `__prev_watch${idx}`, initOnly: true }, `winit${idx}`);
      }
    }
