...
export function generateComponent(parseResult, options = {}) {
  ...
  // Also emits update:propName for Vue v-model compatibility (Vue listens on the element directly)
  // Also emits propNameChange for Angular [(prop)] compatibility
  for (const md of modelDefs) {
    ...
    lines.push(`    }));`);
    // Emit update:propName directly for Vue v-model (no adapter needed)
    lines.push(`    this.dispatchEvent(new CustomEvent('update:${md.name}', { detail: newVal }));`);
    // Emit propNameChange directly for Angular [(prop)] (no adapter needed)
    lines.push(`    this.dispatchEvent(new CustomEvent('${md.name}Change', { detail: newVal }));`);
    lines.push('  }');
    lines.push('');
  }
