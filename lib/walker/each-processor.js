...
export function parseEachExpression(expr) {
  ...
  if (!/\bin\b/.test(expr)) {
    const error = new Error("each requires 'item in source' or '(item, index) in source' syntax");
  ...
  if (destructuredMatch) {
    ...
    if (!itemVar) {
      const error = new Error('each requires an iteration variable');
      /** @ts-expect-error — custom error code */
      error.code = 'INVALID_V_FOR';
      throw error;
    }
    if (!source) {
      const error = new Error('each requires a source expression');
      /** @ts-expect-error — custom error code */
      error.code = 'INVALID_V_FOR';
      throw error;
    }
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (el.hasAttribute('each')) {
        ...
        if (el.hasAttribute('if')) {
          const error = new Error('each and if cannot be used on the same element');
          /** @ts-expect-error — custom error code */
