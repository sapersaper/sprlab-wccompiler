...
/**
 * Build an IfBlock from a completed conditional chain.
 * Replaces chain elements with a comment anchor, extracts branch templates
 * via walkBranch, and computes the anchor path.
 *
  ...
  function findIfChains(node, currentPath) {
    ...
    for (const child of children) {
      ...
      if (hasIf && (hasElse || hasElseIf)) {
        const error = new Error('Conditional directives are mutually exclusive on the same element');
      ...
      if (hasShow && hasIf) {
        const error = new Error('show and if cannot be used on the same element');
        /** @ts-expect-error — custom error code */
        error.code = 'CONFLICTING_DIRECTIVES';
        throw error;
      }
      ...
      if (hasElse && el.getAttribute('else') !== '') {
        const error = new Error('else does not accept an expression');
        /** @ts-expect-error — custom error code */
    ...
    for (const child of children) {
      ...
      if (hasIf) {
        ...
        if (!currentChain || !prevElement || !isChainPredecessor(prevElement)) {
          const error = new Error('else-if/else requires a preceding if at the same level');
          /** @ts-expect-error — custom error code */
