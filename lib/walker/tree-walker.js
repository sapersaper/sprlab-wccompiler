...
/**
 * Override the HTML parser used by walkBranch.
 * In Node.js, linkedom is used by default. In the browser, pass a function
 * that returns { document } from DOMParser.
 * @param {(html: string) => { document: Document }} fn
 */
export function setParseHTML(fn) {
  _parseHTML = fn;
}

let _parseHTML = null;
function getParseHTML() {
  return _parseHTML || linkedomParseHTML;
}
...
export function walkBranch(html, signalNames, computedNames, propNames, constantNames = new Set(), knownTags = new Set()) {
  const { document } = getParseHTML()(`<div id="__branchRoot">${html}</div>`);
