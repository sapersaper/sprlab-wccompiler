...
batch(() => {
  ...
  lastName.set('Doe')
      ...
      expect(result).toContain('__batch(');
      expect(result).toContain("this._state.firstName = 'John'");
      expect(result).toContain("this._state.lastName = 'Doe'");
    });
...
batch(() => {
  ...
  name.set('test')
      ...
      expect(result).toContain("this._state.count = x + 1");
      expect(result).toContain("this._state.name = 'test'");
    });
  });
});
