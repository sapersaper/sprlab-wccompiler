...
export function buildDepGraph(parseResult, transformContext) {
  ...
  // ── Process nested forBlocks inside if-blocks ──
  for (const ifBlock of ifBlocks) {
    for (const branch of ifBlock.branches) {
      for (const nestedFor of (branch.forBlocks || [])) {
        const { source, itemVar, indexVar } = nestedFor;
        // Register renderEach for source signal deps
        const nestedSourceDeps = extractDeps(source, signalNames, propNames, modelDefs);
        for (const dep of nestedSourceDeps) {
          if (dep !== itemVar && dep !== indexVar) {
            addDep(dep, { type: 'renderEach', eachBlockIndex: nestedFor._renderIndex });
          }
        }
        // Process internal bindings/if-blocks/for-blocks recursively
        processIfNestedForBlock(nestedFor, nestedFor._renderIndex, nestedFor.itemVar, nestedFor.indexVar);
      }
    }
  }

  // Helper: process nested forBlocks inside if-blocks (uses _renderIndex instead of parentIdx)
  function processIfNestedForBlock(nestedFor, ownIdx, nestedItemVar, nestedIndexVar) {
    ...
    for (const b of (nestedFor.bindings || [])) {
      ...
      for (const dep of deps) {
        ...
        addDep(dep, { type: 'renderEach', eachBlockIndex: ownIdx });
      }
    }
    for (const sb of (nestedFor.showBindings || [])) {
      ...
      for (const dep of deps) {
        if (dep === niv || dep === nidx) continue;
        addDep(dep, { type: 'showEach', eachBlockIndex: ownIdx, expr, depKey: dep, itemVar: niv, indexVar: nidx ?? null });
      }
    }
    for (const ifBlock of (nestedFor.ifBlocks || [])) {
      ...
      for (const branch of ifBlock.branches) {
        ...
        for (const innerNested of (branch.forBlocks || [])) {
          processIfNestedForBlock(innerNested, ownIdx, innerNested.itemVar, innerNested.indexVar);
        }
      }
    }
  ...
  for (let idx = 0; idx < dynamicComponents.length; idx++) {
    ...
    // Also check computed names (computed values are in _state, extractDeps only checks signals)
    for (const cName of computedNames) {
      if (new RegExp(`\\b${escapeRegex(cName)}\\b`).test(dyn.isExpression)) {
        tagDeps.add(cName);
      }
    }
