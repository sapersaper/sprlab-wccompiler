...
<div>
  ...
  it('nested loops with mixed content produce correct paths', async () => {
    ...
    // item.name and item.inStock should have different paths
    const lines = code.split('\n');
    const nameLines = lines.filter(l => l.includes('item.name') && l.includes('textContent'));
    const stockLines = lines.filter(l => l.includes('item.inStock') && l.includes('textContent'));

    expect(nameLines.length).toBeGreaterThan(0);
    expect(stockLines.length).toBeGreaterThan(0);

    // Extract paths
    const namePath = nameLines[0].match(/(childNodes\[\d+\](?:\.childNodes\[\d+\])*)/);
    const stockPath = stockLines[0].match(/(childNodes\[\d+\](?:\.childNodes\[\d+\])*)/);

    expect(namePath).not.toBeNull();
    expect(stockPath).not.toBeNull();
    // They must point to different nodes
    expect(namePath[1]).not.toBe(stockPath[1]);
  });
});
