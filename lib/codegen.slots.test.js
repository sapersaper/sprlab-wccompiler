...
describe('codegen slots — unit tests', () => {
  ...
  it('scoped slot — generates template storage and reactive effect', () => {
    ...
    expect(output).toContain('item: this._state.currentItem');
  ...
  it('scoped slot with multiple props — generates all prop references', () => {
    ...
    expect(output).toContain('item: this._state.currentItem');
    expect(output).toContain('index: this._state.currentIndex');
  });
  ...
  it('scoped slot with computed source — uses _state. prefix', () => {
    ...
    expect(output).toContain('name: this._state.fullName');
  });
  ...
  it('scoped slot with prop source — uses _s_ prefix', () => {
    ...
    expect(output).toContain('heading: this._state.title');
  });
  ...
  it('mixed slots — named, default, and scoped all together', () => {
    ...
    expect(output).toContain('if (this.__slotTpl_data)');
    expect(output).toContain('item: this._state.current');
  });
