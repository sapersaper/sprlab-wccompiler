...
import { walkTree, walkBranch } from './tree-walker.js';
import { generateComponent, transformExpr, transformForExpr } from './codegen.js';
...
describe('generateComponent — constant text bindings', () => {
  ...
  it('generates __effect for signal bindings but not for constants', () => {
    ...
    expect(output).toContain('this.__text_count_0.textContent = this._count()');
  ...
  it('generates correct code for constants inside if-block branches', () => {
    ...
    // Constant in if-block should be direct assignment (no __effect)
    expect(output).toContain("__text_TITLE_0.textContent = this._const_TITLE ?? '';");
    // Should NOT be wrapped in __effect
    const lines = output.split('\n');
    const titleLine = lines.find(l => l.includes('__text_TITLE_0.textContent'));
    expect(titleLine).not.toContain('__effect');
  });
});
...
describe('generateComponent — full constant interpolation integration', () => {
  it('compiles a component with constants, signals, and methods correctly', () => {
    ...
    // Computed: wrapped in __effect
    expect(output).toContain('this.__text_fullVersion_2.textContent = this._c_fullVersion()');

    // Signal: wrapped in __effect
    expect(output).toContain('this.__text_count_3.textContent = this._count()');
