...
<my-child model:${propName}="${compSignal}"></my-child>
            ...
            expect(code).toContain(`.value = this._state.${formSignal}`);
            ...
            const formBindingForCompSignal = code.includes(`this._state.${compSignal} = e.target.value`) ||
              code.includes(`this._state.${compSignal} = e.target.checked`);
            ...
            const compBindingForFormSignal = code.includes(`addEventListener('wcc:model'`) &&
              code.includes(`this._state.${formSignal}`);
            ...
            if (code.includes("addEventListener('wcc:model'")) {
              ...
              expect(listenerCode).not.toContain(`this._state.${formSignal} =`);
            }
...
<another-comp model:${propName}="${compSignal}"></another-comp>
            ...
            expect(code).not.toContain(`this._state.${compSignal} = e.target.value`);
            expect(code).not.toContain(`this._state.${compSignal} = e.target.checked`);
    ...
    fc.assert(
      fc.property(
        ...
        (varName, propName, signalName) => {
          ...
          // The signal read should use this._state.signalName
          expect(result).toContain(`this._state.${signalName}`);
          // Should NOT use _m_ for the write
          expect(result).not.toMatch(new RegExp(`this\\._m_${propName}\\(this\\._state\\.${signalName}\\)`));
        }
      ),
      { numRuns: 100 }
    );
...
<div>test</div>
            ...
            for (const md of modelDefs) {
              ...
              // (b) Model prop initialized in the Proxy initial object
              expect(code).toContain(`${md.name}:`);
              expect(code).toContain('this._state = new Proxy(');

              // (c) Public get accessor
              expect(code).toContain(`get ${md.name}() { return this._state.${md.name}; }`);

              // (c) Public set accessor
              expect(code).toContain(`set ${md.name}(val) { this._state.${md.name} = val; this.setAttribute('${kebabName}', String(val)); }`);
            }
          } finally {
            try { rmSync(dir, { recursive: true, force: true }); } catch { /* ignore */ }
          }
...
<div>test</div>
            ...
            for (const md of modelDefs) {
              ...
              // Assert: reads old value from the model state
              expect(code).toContain(`const oldVal = this._state.${md.name};`);

              // Assert: writes new value to the model state
              expect(code).toContain(`this._state.${md.name} = newVal;`);
...
<div>test</div>
            ...
            for (const md of modelDefs) {
              // Assert: attributeChangedCallback contains direct state write this._state.{name} =
              expect(callbackBody).toContain(`this._state.${md.name} =`);
...
describe('Feature: define-model, Property 4: model:propName generates bidirectional binding', () => {
  ...
  it('generated code contains addEventListener for wcc:model updating parent signal', () => {
    fc.assert(
      fc.asyncProperty(
        arbBinding,
...
<my-child model:${propName}="${signalName}"></my-child>
            ...
            const kebabPropName = propName.replace(/([a-z0-9])([A-Z])/g, '$1-$2');

            // (a) Assert: kebab-case prop name in addEventListener string
            expect(code).toContain(`'${kebabPropName}', this._state.${signalName}`);
            ...
            // (d) Assert: parent signal write this._state.signalName = e.detail.value
            expect(code).toContain(`this._state.${signalName} = e.detail.value`);
          } finally {
            try { rmSync(dir, { recursive: true, force: true }); } catch { /* ignore */ }
          }
        }
