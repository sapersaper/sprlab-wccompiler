...
/**
 * Recompute anchor paths from the processed HTML to ensure they match
 * the serialized DOM (innerHTML may introduce whitespace differences
 * compared to the temporary DOM used during walkBranch).
 */
function recomputePathsFromProcessedHtml(processedHtml, forBlocks, ifBlocks, dynamicComponents) {
  if (!processedHtml) return;
  if (forBlocks.length === 0 && ifBlocks.length === 0 && dynamicComponents.length === 0) return;

  const { document } = parseHTML(`<div>${processedHtml}</div>`);
  const root = document.querySelector('div');

  function collectComments(node, acc = []) {
    for (const child of node.childNodes) {
      if (child.nodeType === 8) acc.push(child);
      else if (child.nodeType === 1) collectComments(child, acc);
    }
    return acc;
  }

  const allComments = collectComments(root);

  function recompute(items) {
    let commentIdx = 0;
    for (const item of items) {
      if (!item._anchorNode || item._anchorNode.textContent == null) continue;
      const needle = item._anchorNode.textContent.trim();
      while (commentIdx < allComments.length) {
        const c = allComments[commentIdx];
        commentIdx++;
        if (c.textContent.trim() === needle) {
          item.anchorPath = recomputeAnchorPath(root, c);
          break;
        }
      }
    }
  ...
  // Each category starts searching from the beginning
  recompute(forBlocks);
  let savedIdx = 0;  // not shared — each call restarts
}
 ...
 * @param {Set<string>} [constantNames] — Set of constant variable names
 ...
 */
export function walkTree(rootEl, signalNames, computedNames, propNames = new Set(), constantNames = new Set()) {
  /** @type {Binding[]} */
  const bindings = [];
  /** @type {EventBinding[]} */
  const events = [];
 ...
 */
export function walkBranch(html, signalNames, computedNames, propNames, constantNames = new Set()) {
  ...
  const result = walkTree(branchRoot, signalNames, computedNames, propNames, constantNames);
  ...
  // Strip first path segment from nested forBlock/ifBlock/dynamicComponent anchor paths
  ...
  // Recompute anchor paths from the processed HTML to ensure they match
  // the serialized DOM (innerHTML may introduce whitespace differences).
  recomputePathsFromProcessedHtml(processedHtml, forBlocks, ifBlocks, dynamicComponents);
  // Strip again — recomputePaths re-added the wrapper segment
  stripFirstAnchorSegment(forBlocks);
  stripFirstAnchorSegment(ifBlocks);
  stripFirstAnchorSegment(dynamicComponents);
...
function buildIfBlock(chain, parent, parentPath, idx, signalNames, computedNames, propNames) {
  ...
  const branches = chain.branches.map((branch) => {
    ...
    const { bindings, events, showBindings, attrBindings, modelBindings, slots, childComponents, forBlocks, ifBlocks, dynamicComponents, processedHtml } = walkBranch(templateHtml, signalNames, computedNames, propNames);
  ...
  function findForElements(node, currentPath) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (el.hasAttribute('each')) {
        ...
        // Process internal bindings/events via partial walk
        const { bindings, events, showBindings, attrBindings, modelBindings, slots, childComponents: forChildComponents, forBlocks: nestedForBlocks, ifBlocks: nestedIfBlocks, dynamicComponents: nestedDynamicComponents, processedHtml } = walkBranch(templateHtml, signalNames, computedNames, propNames);
 ...
 */
export function processDynamicComponents(parent, parentPath, signalNames = new Set(), computedNames = new Set(), propNames = new Set()) {
  ...
  function findDynamicComponents(node, currentPath) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (el.tagName === 'COMPONENT') {
        ...
        const innerResult = innerHtml ? walkBranch(innerHtml, signalNames, computedNames, propNames) : null;
