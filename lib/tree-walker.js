...
function buildIfBlock(chain, parent, parentPath, idx, signalNames, computedNames, propNames) {
  ...
  const branches = chain.branches.map((branch) => {
    ...
    const { bindings, events, showBindings, attrBindings, modelBindings, slots, childComponents, forBlocks, ifBlocks, processedHtml } = walkBranch(templateHtml, signalNames, computedNames, propNames);
