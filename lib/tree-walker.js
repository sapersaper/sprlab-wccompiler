...
export function walkTree(rootEl, signalNames, computedNames, propNames = new Set()) {
  ...
  function walk(node, pathParts) {
    ...
    if (node.nodeType === 1) {
      ...
      // Detect if attribute (for conditionals inside loops)
      if (el.hasAttribute('if')) {
        const varName = `__if_inline_${showIdx}`; // Reuse showIdx counter for unique names
        showIdx++;
        showBindings.push({
          varName,
          expression: el.getAttribute('if'),
          path: [...pathParts],
          isConditional: true, // Flag to indicate this should generate inline conditional logic
        });
 ...
 */
export function walkBranch(html, signalNames, computedNames, propNames, skipIfChains = false) {
  ...
  // Skip processIfChains when inside a loop context to preserve if elements for inline codegen
  const ifBlocks = skipIfChains ? [] : processIfChains(branchRoot, [], signalNames, computedNames, propNames);
...
function buildIfBlock(chain, parent, parentPath, idx, signalNames, computedNames, propNames) {
  ...
  const branches = chain.branches.map((branch) => {
    ...
    // Pass skipIfChains=true to preserve if elements inside loops for inline codegen
    const { bindings, events, showBindings, attrBindings, modelBindings, slots, childComponents, processedHtml } = walkBranch(templateHtml, signalNames, computedNames, propNames, true);
  ...
  function findForElements(node, currentPath) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (el.hasAttribute('each')) {
        ...
        // Process internal bindings/events via partial walk
        // Pass skipIfChains=true to preserve if elements inside loops for inline codegen
        const { bindings, events, showBindings, attrBindings, modelBindings, slots, childComponents: forChildComponents, forBlocks: nestedForBlocks, ifBlocks: nestedIfBlocks, processedHtml } = walkBranch(templateHtml, signalNames, computedNames, propNames, true);
