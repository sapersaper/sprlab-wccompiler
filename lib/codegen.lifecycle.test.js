  ...
  it('disconnectedCallback is present only when destroy hooks exist, with bodies in order', () => {
    ...
    fc.assert(
      fc.property(arbParseResultWithHooks, (ir) => {
        ...
        // disconnectedCallback is always present (AbortController cleanup)
    ...
    fc.assert(
      fc.property(
        ...
        (tagName, signalName, computedName, hookType) => {
          ...
          // Signal read: x() → this._state.x
          expect(output).toContain(`this._state.${signalName}`);
          // Signal write: x.set(value) → this._state.x = this._state.x + 1
          expect(output).toContain(`this._state.${signalName} = this._state.${signalName} + 1`);
          // Computed read: x() → this._state.x
          expect(output).toContain(`this._state.${computedName}`);
        }
      ),
      { numRuns: 100 }
    );
...
describe('Codegen lifecycle hooks — unit tests', () => {
  it('disconnectedCallback always present with AbortController cleanup (Req 5.4)', () => {
    const ir = {
      tagName: 'wcc-test',
      className: 'WccTest',
      template: '<div>test</div>',
    ...
    const output = generateComponent(ir);
    // disconnectedCallback is always generated for AbortController cleanup
    expect(output).toContain('disconnectedCallback');
    expect(output).toContain('this.__ac.abort()');
  });
