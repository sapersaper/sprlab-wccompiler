...
<div :class="{ active: isActive() }">Content</div>
    ...
    // Should NOT generate invalid syntax like { this._state.isActive: this._state.isActive }
    expect(code).not.toContain('this._state.isActive:');
...
<div :class="{
  ...
  success: !hasError() && size() !== 'large'
    ...
    expect(code).toContain('this._state.hasError');
...
<div :class="[customClass(), size()]">Array classes</div>
    ...
    // Array syntax should use className with join(' ') not join(',')
    expect(code).toContain('.className =');
    expect(code).toContain('this._state.customClass');
    expect(code).toContain('this._state.size');
    
    // Should NOT have comma-separated classes (the bug)
    // The generated code should handle arrays properly
    console.log('\n=== Generated Code for Array Syntax ===');
    const classEffectStart = code.indexOf('this.__attr_class_');
    if (classEffectStart !== -1) {
