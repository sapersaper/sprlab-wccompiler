...
<div>
  <p>{{props.active ? 'Componente ACTIVO' : 'Componente INACTIVO'}}</p>
    ...
    // Must call the signal: this._s_active() (with parentheses)
    // NOT reference the function: this._s_active (without parentheses, always truthy)
    expect(code).toContain("this._s_active() ? 'Componente ACTIVO' : 'Componente INACTIVO'");
...
<div>
  <p>{{props.title}}</p>
    ...
    expect(code).toContain("this._s_title() ?? ''");
  });
...
<div>
  <p>{{props.count > props.max ? 'Over limit' : 'Within limit'}}</p>
    ...
    expect(code).toContain('this._s_count()');
    expect(code).toContain('this._s_max()');
...
<div>
  <p>{{props.active && props.visible ? 'Show' : 'Hide'}}</p>
    ...
    // Both props should be called as signals
    expect(code).toContain('this._s_active()');
    expect(code).toContain('this._s_visible()');

    // The expression should be wrapped in parentheses (contains ternary + &&)
    expect(code).toMatch(/\(this\._s_active\(\) && this\._s_visible\(\) \? 'Show' : 'Hide'\)/);
  });
...
<div>
  ...
  <p>{{statusText()}}</p>
    ...
    expect(code).toContain("this._s_active() ? 'ACTIVO' : 'INACTIVO'");

    // Computed should also work correctly
    expect(code).toContain('this._c_statusText()');
  });
...
<div>
  <div if="showDetails()">
    ...
    expect(code).toContain("this._s_active() ? 'ON' : 'OFF'");
...
<div>
  <p>{{props.status === 'loading' ? 'Cargando...' : 'Listo'}}</p>
    ...
    expect(code).toContain("this._s_status() === 'loading'");
...
<div>
  <p>{{!props.disabled ? 'Enabled' : 'Disabled'}}</p>
    ...
    expect(code).toContain('this._s_disabled()');
...
<div>
  <p>{{getLabel()}}</p>
    ...
    // Method calls should still work (baseName strips (), codegen adds it back)
    expect(code).toContain('this._getLabel()');
  });
});
