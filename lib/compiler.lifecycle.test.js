...
<div>{{count()}}</div>
    ...
    // Signal read transformed: count() → this._state.count
    expect(output).toContain("console.log('mounted, count is', this._state.count)");
    // Signal write transformed: count.set(count() + 1) → this._state.count = this._state.count + 1
...
<div>{{doubled()}}</div>
    ...
    // Computed read transformed: doubled() → this._state.doubled
