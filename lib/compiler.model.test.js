...
describe('compile() — model directive end-to-end', () => {
  it('compiles a component with multiple model directives (text, checkbox, radio, number, select, textarea)', async () => {
    ...
    try {
      writeFileSync(join(dir, 'form.html'), [
        '<input model="name">',
      ...
      writeFileSync(join(dir, 'form.js'), `import { defineComponent, signal } from 'wcc'
...
`);
      ...
      const output = await compile(join(dir, 'form.js'));
  ...
  it('compiles model alongside interpolation and event bindings', async () => {
    const dir = createTempDir();
    try {
      writeFileSync(join(dir, 'mixed.html'), [
        '<div>{{greeting}}</div>',
        '<input model="name">',
        '<button @click="submit">Submit</button>',
      ].join('\n'));

      writeFileSync(join(dir, 'mixed.js'), `import { defineComponent, signal } from 'wcc'
...
`);
      ...
      const output = await compile(join(dir, 'mixed.js'));
