    ...
    fc.assert(
      fc.property(
        ...
        (itemVar, signalName) => {
          ...
          expect(output).toContain(`__effect(() => { node.childNodes[2].textContent = this._state.${signalName}`);
        }
      ),
      { numRuns: 100 }
    );
...
describe('codegen each — unit tests', () => {
  ...
  it('generates reactive show binding for component-level expression', () => {
    ...
    expect(output).toContain("__effect(() => { node.childNodes[0].style.display = (this._state.showAll) ? '' : 'none'; })");
  });
});
