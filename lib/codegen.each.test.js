    ...
    fc.assert(
      fc.property(
        ...
        (itemVar, source, signalName) => {
          ...
          // items array initialized
          expect(output).toContain('this.__for0_items = []');

          // __invalidate calls __renderEach_0
          expect(output).toContain('this.__renderEach_0();');
        }
      ),
      { numRuns: 100 }
    );
    ...
    fc.assert(
      fc.property(
        ...
        (itemVar, signalName) => {
          ...
          // Reactive binding: static assignment in __renderEach_0 (no __effect wrapper around internal bindings)
          expect(output).toContain(`node.childNodes[2].textContent = this._state.${signalName} ?? ''`);
        }
      ),
      { numRuns: 100 }
    );
...
describe('Feature: each-directive, Property 6: transformForExpr Preserves Item/Index and Transforms Component Refs', () => {
  ...
  it('transforms signals to this._state.x while preserving item/index', () => {
    fc.assert(
      fc.property(
        identArb,
        identArb,
  ...
  it('transforms computeds to this._state.x while preserving item/index', () => {
    fc.assert(
      fc.property(
        identArb,
        identArb,
        (itemVar, computedName) => {
          ...
          expect(result).toContain(`this._state.${computedName}`);
          expect(result).not.toContain(`this._state.${itemVar}`);
        }
      ),
      { numRuns: 100 }
    );
  ...
  it('transforms props to this._state.x while preserving item/index', () => {
    fc.assert(
      fc.property(
        identArb,
        identArb,
        (itemVar, propName) => {
          ...
          expect(result).toContain(`this._state.${propName}`);
          expect(result).not.toContain(`this._state.${itemVar}`);
        }
      ),
      { numRuns: 100 }
...
describe('codegen each — unit tests', () => {
  it('handles numeric range: source is a number N → iterate 1 through N (Req 7.5)', () => {
    ...
    const output = generateComponent(pr);
    // Source transformed as literal number in __renderEach_0
    expect(output).toContain('__renderEach_0() {');
  ...
  it('generates reactive show binding for component-level expression', () => {
    ...
    const output = generateComponent(pr);
    // Show binding: static assignment in __renderEach_0, bulk update in __invalidate
    expect(output).toContain("node.childNodes[0].style.display = (this._state.showAll) ? '' : 'none'");
    expect(output).toContain('this.__for0_nodes[__i].style.display');
  });
});
