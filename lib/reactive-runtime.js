 ...
 *
 * @param {{ needsSignal?: boolean, needsComputed: boolean, needsEffect: boolean, needsBatch: boolean, needsUntrack: boolean }} usage
 * @returns {string}
 */
export function buildInlineRuntime(usage) {
  let code = '';
  // Only include globals if effect, computed, or batch is needed (batch uses __batchDepth, __pendingEffects)
  if (usage.needsEffect || usage.needsComputed || usage.needsBatch) {
    code += runtimeGlobals;
  }
  // Only include __signal if explicitly needed
