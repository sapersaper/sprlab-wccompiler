...
<div>
  ...
  <button @click="toggle">Toggle</button>
      ...
      expect(output).toContain('this.__show_0 = __root.');
      expect(output).toContain('this.__show_1 = __root.');
      ...
      const show0Pos = ccSection.indexOf('this.__show_0 = __root');
      const show1Pos = ccSection.indexOf('this.__show_1 = __root');
      ...
      expect(output).toContain('this.__show_0.style.display = (this._state.isVisible) ?');
      expect(output).toContain('this.__show_1.style.display = (this._state.count > 0) ?');

      // 5. Display toggle pattern
      expect(output).toContain("? '' : 'none'");

      // 6. Signal initialization in Proxy state
      expect(output).toContain('isVisible:');
      expect(output).toContain('count:');
      expect(output).toContain('message:');

      // 7. Text binding for {{message()}}
      expect(output).toContain('this._state.message');
...
<div show="count > 0">Has items</div>
      ...
      expect(output).toContain('this._state.count > 0');
