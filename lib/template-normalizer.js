...
export function normalizeTemplate(html, options) {
  ...
  // BUG-0014 FIX: Pre-process ALL Mustache-style attribute bindings
  // Convert attr={{ expr }} and attr="{{ expr }}" to remove Mustache delimiters
  // The HTML parser breaks comparison operators and leaves raw {{ in generated code
  
  // Pattern 1: Quoted syntax - attr="{{ expr }}"
  // Handles: if, else-if, @click, :is, :key, etc.
  html = html.replace(/\b([\w:-]+)\s*=\s*"\{\{([^}]+)\}\}"/g, '$1="$2"');
  
  // Pattern 2: Unquoted syntax - attr={{ expr }}
