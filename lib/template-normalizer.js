...
export function normalizeTemplate(html, options) {
  ...
  // BUG-0014 FIX: Pre-process Mustache-style conditional bindings
  // Convert if="{{ expr }}" and else-if="{{ expr }}" to remove Mustache delimiters
  // The HTML parser breaks comparison operators inside {{ }}, so we extract the expression
  
  // Pattern 1: if="{{ expr }}" → if="expr"
  html = html.replace(/\bif\s*=\s*"\{\{([^}]+)\}\}"/g, 'if="$1"');
  
  // Pattern 2: else-if="{{ expr }}" → else-if="expr"
  html = html.replace(/\belse-if\s*=\s*"\{\{([^}]+)\}\}"/g, 'else-if="$1"');
  
  // Pattern 3: if={{ expr }} (without quotes) - handle unquoted syntax
  // This prevents HTML parser from breaking it into malformed attributes
  html = html.replace(/\bif\s*=\s*\{\{([^}]+)\}\}/g, 'if="$1"');
  
  // Pattern 4: else-if={{ expr }} (without quotes)
  html = html.replace(/\belse-if\s*=\s*\{\{([^}]+)\}\}/g, 'else-if="$1"');
